...
class livenow extends CI_Controller{
	...
	public function GetData(){
		$Data=$this->livenow_model->GetArticleData(731 , $this->input->post('search'));
		$Content=array();
		$Fullcontent=array();
		...
		if(!file_exists($path.$FileName)){
			...
			foreach($Data as $DataValue){
				...
				if($DataValue['status']==1){
					$template .='<button class="btn btn-primary del" onclick="delete_content('.$i.')"><i class="fa fa-trash-o" aria-hidden="true"></i></button>'; 
				}else{
	...
	public function PutFile($FilePath,$content){
		...
		//curl_setopt($ch, CURLOPT_URL, BASEURL.'user/commonwidget/Content_file_intimation');
                curl_setopt($ch, CURLOPT_URL, 'http://www.dinamani.com/user/commonwidget/Content_file_intimation');
