...
class Template_designer extends CI_Controller {
		...
		if($design_lock_status == 2) /* Template is unlocked */
		{
			...
			if($publish_xml == "publish")
			{
				...
				if($section_id!=10000 && $section_id!=10001){
					...
					$trendingHours = [6 , 9 , 12 , 15 , 18 ,21 ,11];
					$currentTime = date('H');
					$currentMin = date('i');
					if(in_array($currentTime, $trendingHours) && $currentMin <=30){
						$this->trending_tags();	
					}
	...
	public function save_html_template($menuid , $pagetype)
	{
		...
		if($xml != '' && !empty($xml))
		{
			...
			for($i=1; $i<= $loop_break_point; $i++){
				...
				$c_class_value 	= " col-lg-".$section_cl_val." col-md-".$section_cl_val." col-sm-".$section_cl_val." col-xs-".$col_xs_val." ".$home_last_column." ";
	...
	public function curl_post($file_name, $file_data)
	{
		...
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, 'http://www.dinamani.com/user/commonwidget/post_file_intimation');
	...
	public function trending_tags(){
		...
		$endDate = date('Y-m-d').' 23:59:58';
		$response .='<div class="row">';
		$response .='<div class="col-xs-12 col-sm-2 col-md-2 col-lg-2">';
		$response .='<p class="title">TRENDING TODAY</p>';
		$response .='</div>';
		$response .='<div class="trending-wrapper">';
		$response .= $this->form_tags($this->widget_model->trending_tagslist($fromDate , $endDate ,1));
		$response .='</div>';
		$response .='</div>';
		$response .='<div class="row">';
		$response .='<div class="col-xs-12 col-sm-2 col-md-2 col-lg-2">';
		$response .='<p class="title">TRENDING WEEK</p>';
		$response .='</div>';
		$response .='<div class="trending-wrapper">';
		$response .= $this->form_tags($this->widget_model->trending_tagslist(date("Y-m-d", strtotime("-1 week")).' 00:00:00' , date('Y-m-d').' 23:59:59'));
		$response .='</div>';
		$response .='</div>';
		$response .='<div class="row">';
		$response .='<div class="col-xs-12 col-sm-2 col-md-2 col-lg-2">';
		$response .='<p class="title">LATEST NEWS</p>';
		$response .='</div>';
		$response .='<div class="trending-wrapper">';
		$response .= $this->form_tags($this->widget_model->trending_tagslist('' ,'' ,2));
		$response .='</div>';
		$response .='</div>';
		$response .='<div class="row">';
		$response .='<div class="col-xs-12 col-sm-2 col-md-2 col-lg-2">';
		$response .='<p class="title">LATEST SECTIONS</p>';
		$response .='</div>';
		$response .='<div class="trending-wrapper">';
		$response .='<a target="_BLANK" href="'.BASEURL.'tamilnadu"><i class="fa fa-play"></i> தமிழ்நாடு</a>';
		$response .='<a target="_BLANK" href="'.BASEURL.'india"><i class="fa fa-play"></i> இந்தியா</a>';
		$response .='<a target="_BLANK" href="'.BASEURL.'latest-news"><i class="fa fa-play"></i> தற்போதைய செய்திகள்</a>';
		$response .='<a target="_BLANK" href="'.BASEURL.'cinema"><i class="fa fa-play"></i> சினிமா</a>';
		$response .='<a target="_BLANK" href="'.BASEURL.'employment"><i class="fa fa-play"></i> வேலைவாய்ப்பு</a>';
		$response .='</div>';
		$response .='</div>';

		$data['sectionfname'] = 'trendingsection.php';
		$data['scontent'] = $response;
		chmod(FCPATH.'application/views/view_template/'.$data['sectionfname'] , 0777);
		file_put_contents(FCPATH.'application/views/view_template/'.$data['sectionfname'] , $data['scontent']);
		$post_data = array('sectionfname' => $data['sectionfname'],'scontent'=> $data['scontent']);
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, 'http://www.dinamani.com/user/commonwidget/save_trending');
		curl_setopt($ch, CURLOPT_POST,1);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
		curl_setopt($ch, CURLOPT_POSTFIELDS, $post_data);
		$result=curl_exec($ch);
	...
	public function form_tags($tags){
		...
		foreach($tags as $tag){
			...
			$result .='<a target="_BLANK" href="'.$url.'"><i class="fa fa-play"></i> '.$list.'</a>';
