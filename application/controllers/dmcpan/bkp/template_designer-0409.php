		...
		if($xml != '' && !empty($xml))
		{
		   ...
		   $viewpage = $this->load->view("admin/view_frontend", $data, true);
		    if((strtolower(@$section_details['Sectionname'])=="home") || (@$section_details['Sectionname'] =="முகப்பு")){        
				$home_file = FCPATH.'application/views/view_template/home.php';
			...
			unlink($folder.'/'.'home.php');
			rename($folder.'/'.$file_name, $folder.'/home.php');
			}
			$home_file = FCPATH.'application/views/view_template/home-cpy.php';
			$handle = fopen($home_file , 'w');
			if(flock($handle, LOCK_EX))
			{
				fwrite($handle, $viewpage);
				flock($handle, LOCK_UN);
			}
			$post_data = array('html_info' => '123456','file_contents'=> $viewpage);
			$ch = curl_init();
