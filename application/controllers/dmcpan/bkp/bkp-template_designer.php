...
class Template_designer extends CI_Controller {
	...
	public function save_html_template($menuid , $pagetype)
	{
		...
		/*$check_file_exist = file_exists(FCPATH.'uploads/page_layouts/'.$page_details['menuid'].'_'.$pagetype.'.xml');
		if($check_file_exist==''):
		$xml = '';
        else:
		$xml = simplexml_load_file(FCPATH.'uploads/page_layouts/'.$page_details['menuid'].'_'.$pagetype.'.xml');   // load from folder path
		endif;*/
