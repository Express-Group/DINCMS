...
class Template_designer extends CI_Controller {
	...
	public function index()
	{
		...
		$this->template_design_model->inactivate_temp_articlecustomdetails('',$user_id); 
		...
		//$this->output->cache(1); // for 1 minute ///
	...
	{
		///////  Verify User Access  ///////////
		/*if( !defined("FPM_ADDPAGEDESIGN")){ redirect("niecpan/"); exit; } 
		else if( !FPM_ADDPAGEDESIGN && !FPM_ADDARTICLEOPTION && !FPM_ADDADVSCRIPTS && !FPM_ADDCONFIG  ){ redirect("niecpan/"); exit;}*/
	}
	...
	public function save_xml_template()
	{
		...
		if($design_lock_status == 2) ////  Template is unlocked  ////
		{
			...
			$use_parent_section_template 		= trim(array_key_exists("use_parent_section_template",$post)?$post['use_parent_section_template']:'');
			  ...
			  //var_dump($use_common_header, $use_common_rightpanel, $use_common_footer);exit;
			//echo $tempStr ; exit;
			...
			if($publish_xml == 'publish')
			{
				...
				$get_adv_instances 				= $this->get_all_instances_by_version_id($version_id, '');	///// =1->only advertisement instances, !=1->All widget instances
				$publishing_widget_instances 	= $get_adv_instances['version_widget_instances']; 
				$publishing_mainConfig			= $this->all_mainSectionConfig_by_version_id($version_id, ''); ///// =1->only advertisement instances, !=1->All widget instances
			...
			//$this->session->set_flashdata('alert_message',"Template saved");
			////////////  Create an XML File  //////////////
			if($publish_xml == "publish")
			{
				...
				//SAVE CONTENT
				//$homepage_details	= $this->template_design_model->get_homePage_xml(); //$section_id==$homepage_details['home_section_id']
				...
				if($page_type== 1 && (strtolower($section_details['Sectionname'])=="home" || $section_details['Sectionname'] =="முகப்பு")){ // HTML Generation to home page 
	...
	public function load_template()
	{
		...
		//print_r($not_saved_version); exit;
		
		if($versionId == ''){
			if($not_saved_version['Version_Id'] == ''){
				$template_details 	= $this->template_design_model->load_published_version_template($pageId); 		/// published template
			}
			else
			{
				$template_details 	= $this->template_design_model->load_template_by_version_id($not_saved_version['Version_Id']); /// Not saved version template
			}
		}
		else
		{
			$load_version_id	= ($not_saved_version['Version_Id'] !='' )? $not_saved_version['Version_Id']:$versionId;
			//$template_xml 	= $this->template_design_model->get_template_xmlcontent($pageId, 'xmldata');  //// old method used ///
		...
		//print_r($template_details); exit;
		$result = array("template_xml" => $template_xml, "is_template_closed_before_save" => $is_template_closed_before_save, "template_lock_status" => $template_lock_status, "template_locked_user" => $template_locked_user, "template_locked_user_name" => $template_locked_user_name, "total_versions" => $total_versions, "is_changes_published" => $is_changes_published); //is_template_closed_before_save- 1->Yes, 2->No
		header("content-type: application/json"); 
	...
	public function show_widgetconfig()
	{
		...
		//print_r($data['widget_instancemainsection']); exit;
		//echo "<pre>".print_r($data['widget_instancemainsection']); echo "</pre>"; 
		...
		//print_r($instance_subsection_details);
		//exit;
		...
		//echo "<pre>".print_r($data['widget_instancesubsection']); echo "</pre>"; exit;
		//print_r($data['widget_instancesubsection']); exit;
	...
	public function dynamic_containers()
	{
		...
		{		
			...
			//$id = $container_details['containerid'];
			...
			$container_content[$content_id] = '<div class="container-wrapper section group" data-type="'. $type .'" id="container-'. $id .'">' .			
				...
				for($j=1; $j<=$nos; $j++)
				{
					...
					//((FPM_ADDADVSCRIPTS) ? '<a href="javascript:;" class="config-widget-button fa fa-plus" data-target-container="' . $id .'-'. $j . '" title="'.$configWidget.'"></a>' : "").
					
					//////////  previous code having modal - fancy box  ///////////
					///// <a href="#modal1" class="add-widget-button fa fa-plus" id="' . $id .'-'. $j . '" data-target-container="' . $id .'-1' . '" onclick="show_articles(this.id)"  title="'.$addWidget.'" ></a>
					$container_content[$content_id] .= '<div class="col span_'. $span_class_value .'_of_'. $of_class_value .' wc">' .
								...
								'</div>' .		
							'</div>' ;						
					//////<a class="view-widget-button fa fa-eye" href="#"  data-target-container="' . $id .'-'. $j . '"  title="View Widget" ></a>		
	...
	{	
		...
		//PRINT_R($template_version_details); EXIT;
	...
	public function save_html_template($menuid , $pagetype)
	{
		...
		if($xml != '' && !empty($xml))
		{
		   ...
		   $viewpage = $this->load->view("admin/view_frontend", $data, true);
			...
			file_put_contents($file, $viewpage );exit;
		}else if($section_page_id==10000){
           		$tplheader_values 	= $xml->tplcontainer;
			
			$header 	= $this->load_saved_template_data($tplheader_values, 'top', "header", $tmpl_values, '', $is_home_page, $static_view, $viewmode, '', $pagetype, $page_param, "live", $page_details);
			if ($header!=''){
			...
			$body 		= $this->load_saved_template_data($tplheader_values, 'left', "template_body", $tmpl_values, '', $is_home_page, $static_view, $viewmode, '', $pagetype, $page_param,  "live", $page_details);
			if ($body!=''){
	...
	{				
		$i = 1;	
		$image_number = '';
		if($this->input->post("content_ID") == '' && $this->uri->segment(1) != folder_name) /////  For LIVE URL Content
		{			
			...
			if($this->uri->segment(1) == "")
			{
				...
				//////  Load Home page Template details  ///
				$live_query_string	= explode("-","1");
				$page_id			= $live_query_string[0];
			...
			else if($this->uri->segment(1) != folder_name)
			{
				...
				if(is_numeric(@$live_query_string[count($live_query_string)-1]))
				{
					////  It is article page -- @$live_query_string[count($live_query_string)-1] (Last value of querystring is article ID) ////
					$page_type				= "1";				
					
					if(is_numeric(@$live_query_string[count($live_query_string)-2])) ////  Image number will be added at the end of URL, separated with '-'  /////
					{							
						////  last before exploded item will be contentID, last item will be Image number  /////
						$content_id		= @$live_query_string[count($live_query_string)-2]; 
						$image_number	= @$live_query_string[count($live_query_string)-1]; 								
					}
					else
					{
						$content_id		= @$live_query_string[count($live_query_string)-1];
						$image_number	= '';
					}
					//$url_section_details 	= $this->template_design_model->required_widget_content_by_id($content_id, '1');	///  '1' - for articles			
					$content_det = $this->widget_model->get_content_details($content_id)->result_array();
					$url_section_details 	= $this->template_design_model->required_widget_content_by_id($posted_values[0], $content_det[0]['content_type_id']);
					$viewmode		= "live"; 
				}
				else
				{
					...
					switch(count($live_query_string))
					{
						case 1:
							...
							$special_section	= '';
							$url_parent_section = '';
							$url_sub_section 	= join( " ",( explode("-",$live_query_string[0] ) ) );
	
							$url_section_details= $this->template_design_model->get_sectionid_with_names($url_sub_section, $url_parent_section, $special_section);	
						break;
						...
						case 2:
							...
							////  Parent section name = $live_query_string[0], sub section name = $live_query_string[1]  /////
							$special_section	= '';
							$url_parent_section = join( " ",( explode("-",$live_query_string[0] ) ) );
							$url_sub_section 	= join( " ",( explode("-",$live_query_string[1] ) ) );
							$url_section_details= $this->template_design_model->get_sectionid_with_names($url_sub_section, $url_parent_section, $special_section);																			
						break;
						...
						case 3:
							////  Find the section Id with parent section name, sub section name  //////
							////  Parent section name = $live_query_string[0], sub section name = $live_query_string[1]  /////
							$special_section	= join( " ",( explode("-",$live_query_string[0] ) ) );
							$url_parent_section = join( " ",( explode("-",$live_query_string[1] ) ) );
							$url_sub_section 	= join( " ",( explode("-",$live_query_string[2] ) ) );
						...
						{
						 $page_details  = $this->template_design_model->getPageDetails(@$url_section_details[0]['Section_id'], $page_type);
						 $content_id  = "";
						 $viewmode  = "live"; 
						}
						else
		...
		else if($this->uri->segment(1) == folder_name) ////  From Admin Preview
		{
				$page_details 	= array();
				$content_id		= "";
		...
		if($xml != '' && !empty($xml))
		{
			...
			$page_type = $page_details['pagetype'];
			//$content_id = $content_det[0]['content_id'];
		...
		foreach($tplheader_values as $key => $values)
		{
			...
			{								
				...
				if($file_type == "left") ///// only for Body section content
				{
					...
					//echo "<br>".$section_cl_val.":".array_sum($body_section);
					$col_sm_val		= "12";
					$col_xs_val		= "12";
					$home_last_column = "";
				...
				foreach($values->widgetcontainer as $ckey => $cvalues )
				{
					...
					$c_inc = 0;		  
					//print_r($container_layout);exit;
					foreach($cvalues->widget as $wkey => $wvalues)
					{
						//echo "-container_layout-c_inc:".$c_inc."-"; 
						...
						/*}else   // widgets load by ajax call 
						{
							$dat = json_encode($wvalues);
							$templateString['view_templ'] .= '<div class="widget-container widget-container-' .  $wvalues['id'] . '  fill_widget_'.$wvalues['data-widgetinstanceid'].'" id="fill_widget_'.$wvalues['data-widgetinstanceid'].'" data-pageparam="'.$page_param.'" data-contentfrom="'.$content_from.'" data-instanceid="'.$wvalues['data-widgetinstanceid'].'" data-pagesection="'.$page_details['menuid'].'">';
							$templateString['view_templ'] .= '<div id="widget_fill_widget_'.$wvalues['data-widgetinstanceid'].'" style="display:none;">'.($dat).'</div>';
							$templateString['view_templ'] .= '<div class="cssload-container" id="add_article_process_img" style="display:none;"><div class="cssload-zenith"></div></div>
<div class=" ajaxStatus ajaxFailed noDisplay" style="display: none;">Failed to load the content...</div></div>';
	...
	public function widget_instance()
	{
		...
		//$data['widget_instance'] = $this->template_design_model->getWidgetInstance($section_id, $pagetype, $container_id, $widget_id, $widget_disp_order);
	...
	public function show_articles()
	{
		...
		//if($is_related_article == "no" || $is_related_article == "yes")		
		$from_related_article = $this->input->post('from_related_article');
		
		$mainsection_details	= array();
		/*$widget_instance		= $this->uri->segment(4);
		$widget_pageId			= $this->uri->segment(5);*/
		...
		//$home_value				= $this->input->post("home_value");
		...
		//print_r($lock_details); exit;
		//if($lock_details['res_status'] == 1){
		...
		/*if($this->input->post('from_related_article') != "yes" && $related_content_id == 0)
		{
			///////////  Add widget articles to temporary table using widgetinstance  ///////////////
			$temp_insert = $this->widget_articles_temp($widget_instance);					
		...
		//echo "<br> is_related_article :".$is_related_article." -> from_related_article : ".$from_related_article." -> related_content_id: ".$related_content_id;
		$edit_flag 	 = '0';
		...
		//echo  $temp_insert->conn_id->affected_rows; exit;
		//affected_rows
		...
		if($widget_details['widgetStyle'] != 1)
		{
			...
			if(count($main_section_config) > 0 && FPM_ADDARTICLEOPTION)
			{
				...
				///// Release All locks  ////
				//$this->template_design_model->release_locks_by_user_id();								
				$_POST['widgetinstance_id'] = $widget_instance;
				$_POST['return_type'] 		= "php";
				$_POST['from_release_article_lock'] = "from_show_articles"; // for lock_widget_articles()
		...
		//$main_section_config 	= array_unique($main_section_config);
		//print_r($main_section_config ); exit;
		
		$data['edit_flag']				= $edit_flag;
		
		//print_r(array_unique($mainsection_details)); exit;
		
		$data['section_group']			= $this->get_section_group();
		//print_r($data['section_group']); exit;
		$content_type_group				= $this->common_model->get_content_type("");
		...
		{
			//$supported_image_sizes[$position_sizes['Contentposition']] = array('ImageWidth'=> $position_sizes['ImageWidth'], 'ImageHeight'=> $position_sizes['ImageHeight']);
		...
		$this->load->view('admin_template',$data);	
		/* }else{
			if($lock_details['show_msg'] == 1){
				$this->session->set_flashdata(array("alert_message" => $lock_details['msg']));
			}
			redirect(folder_name.'/template_designer/');
	...
	public function get_widgetInstancearticle()
	{
		...
		//print_r($data['contenttype_id']); exit;
		...
		{			
			/////  Old method //////
			//$all_available_articles		= $this->template_design_model->get_all_available_articles($parent_section_id, $data['contenttype_id'], $search_bytitle);			
			$search_value 	= $search_bytitle;
			$search_by 		= ($search_value != '') ? "Title" : "";
		...
		{
			//$all_available_articles		= $this->template_design_model->required_widget_content_by_id($search_bytitle, $data['contenttype_id']); 	
			$search_value 	= $search_bytitle;
...
//echo $this->db->last_query(); exit;
		$widget_instance_articles 	= $this->template_design_model->get_widgetInstanceTempArticles($widget_instance, $mainSectionConfig_Id,  $subSectionConfig_Id, '', $user_id, ''); 		
		
		//echo $this->db->last_query(); exit;
		//print_r($widget_instance_articles	); exit;	
		...
		{	
			...
			/*if($for_content_id['Status'] != '2' && $page_name != "home")
			{
				$instancecontent_idlist[] = $for_content_id['content_id'];
				$widget_instance_active_articles[] = $for_content_id;
				//$instancecontent_statuslist[] = $for_content_id['Status'];
			}
		...
		echo json_encode($instancecontent_idlist); exit;*/
		...
		$edit_flag_articles 	= $this->template_design_model->get_widgetInstanceTempArticles_active($widget_instance, '',  '', '', $user_id, '');
		...
		//print_r($all_available_articles); exit;
	...
	public function get_widgetInstance_related_article()
	{
		...
		$widget_instance_articles 	= $this->template_design_model->get_widgetInstanceTempRelatedArticles($widget_instance, $mainSectionConfig_Id,  $subSectionConfig_Id, '', $user_id, $related_content_id);
		//echo $this->db->last_query(); exit;
		//echo $this->db->last_query(); 	
		//print_r($widget_instance_articles); exit;
	...
	{	
		...
		$instanceId = $this->template_design_model->insert_or_update_WidgetInstance($widget_instance, $user_id); 	
	...
	public function widgetarticle_customdetails()
	{
		...
		//$custom_details['uploaded_image'] 			= $_FILES["uploaded_image"]['tmp_name'];
		//exit;
		...
		if($custom_details['temp_image_id'] != ''){
			...
			if(count($temp_image_save_details) > 0)
			{
				if($temp_image_save_details['imagecontent_id'] != '') ////  Image From Image Library
				{
					...
					//if($temp_image_save_details['save_status'] == 2 || $temp_image_save_details['save_status'] == 1)
					//{
						...
						//echo $custom_details['moved_image_id']; exit;
						
					// }
					// else
					// {
						// $custom_details['moved_image_id'] = $temp_image_save_details['imagecontent_id'];
		...
		$widget_content_id = $this->template_design_model->addwidget_articlecustomdetails_temporary($custom_details, $user_id); 
		...
		$this->template_design_model->display_order_in_sequence($table_name, $custom_details['instanceId'], $custom_details['instanceMainSectionId'],  $custom_details['instanceSubSectionId'], $user_id, @$related_content_id);
		
		
		/////  Move temp table to main table  ////
		 $widget_instance_articles 	= $this->template_design_model->get_widgetInstanceTempArticles_active($custom_details['instanceId'], '',  '', '', $user_id);			 		 //echo $this->db->last_query(); exit;
		 //print_r($widget_instance_articles); exit; 			
	...
	{				
		...
		//exit;
		//echo $is_save_temp; exit;
		//$widget_instance_id = $article_postedvalues[0]['widget_instance_id'];
		$widget_instance_id = $this->input->post('widget_instance_id');
		$related_content_id = $this->input->post('related_content_id'); 
		$version_id			= $this->input->post('version_id'); 
		$live_version_id	= $this->input->post('live_version_id'); 
		...
		{			
			if($widget_instance_id != '')
			{
				...
				//$published_result = $this->template_design_model->publish_widgetinstance_articles($widget_instance_id, $user_id);
				...
				//print_r($publishing_instance_articles); exit;
				//SAVE CONTENT
				$homepage_details	= $this->template_design_model->get_homePage_xml();
		...
		{			
			 ...
			 $first_update_incactive = 1;
				...
				/*echo $custom_details['instanceId'];
				///////////  Remove Temptable content and Add widget articles to temporary table using widgetinstance  ///////////////
				$this->template_design_model->inactivate_temp_articlecustomdetails('',$user_id); 
				$temp_insert = $this->widget_articles_temp($custom_details['instanceId']);				
				//echo $this->db->last_query();*/
		...
		else
		{
			...
			//$temp_article_previous_list = explode(",",$article_postedvalues[0]['temp_article_list']);
			$first_update_incactive = 1;
			...
			//echo "<br> editflag : ".$edit_flag_status;
			...
			foreach($article_postedvalues as $widget_article_details)
			{
				//print_r($widget_article_details); 			
				$custom_details 						= array();
				...
				$custom_details['version_id']			= $version_id;
				//$custom_details['display_order'] 		= ( $widget_article_details["article_priority"] != '' && $widget_article_details["article_priority"] != 0 ) ? $widget_article_details["article_priority"] : (($widget_article_details["article_priority"] != $previous_item_displayorder ) ? $first_update_incactive : ($previous_item_displayorder + $first_update_incactive ) );
				//$first_update_incactive					= $previous_item_displayorder + 1;
				//$previous_item_displayorder				= $custom_details['display_order'];
				...
				//$custom_details['edit_flag']			= @$widget_article_details['edit_flag'];
				...
				if($is_save_temp_exp[0] == 'saveTemporary')
				{
					...
					if($first_update_incactive == 1)
					{
						//$this->template_design_model->inactivate_instance_temparticles_status($custom_details, $user_id); 													
					}
					//exit;
					...
					//$updated_temp_article = $this->template_design_model->get_widgetInstanceTempArticles('','',$custom_details['instanceId'],$custom_details['instanceMainSectionId'],$custom_details['instanceSubSectionId'], $custom_details['content_id'], $user_id, $custom_details['related_content_id']); 	
					//print_r($updated_temp_article); 
					/*$is_checked_article = array_search($updated_temp_article[0]['instancecontent_tempID'], $temp_article_previous_list);
					if(!is_bool($is_checked_article))
					{
						unset($temp_article_previous_list[$is_checked_article]);
					}*/
					/////////////////////////////////////////////////////////////////////////////////
			...
			//print_r($temp_article_previous_list); exit;
	...
	public function upload_image($upload_path, $image_control_name )
	{
		...
		$config = array(
			//'upload_path' 		=> "uploads/imagelibrary/temp",
	...
	{		
		...
		//print_r($instance_subsection_details); exit;
		header("content-type : application/json");
	...
	{		
		...
		$widget_instance_temparticles 	= $this->template_design_model->insert_temporary_from_instance_articles ($current_widgetinstance, $user_id, $version_id); 	
		...
		if(FPM_ADDPAGEDESIGN)
		{
			...
			{			
				...
				if($this->input->post("deleteall") == 'y')
				{
					...
					{
						//$deleted_widget 		= $this->template_design_model->remove_widget_instance($widget_instance, '', '', $user_id); 
						/////  Do the changes in temporary tables  ////
				...
				else if($this->input->post("deleteTemplate") == 'y')
				{
					...
					$section_id 	= $page_details['menuid']; 
					$page_type		= $page_details['pagetype'];
					$deleted_widget	= $this->template_design_model->remove_widget_instance('', $section_id, $page_type, $user_id); 	
					header("content-type: application/json");
					echo json_encode($deleted_widget);
					exit;
				}
	...
	{		
		if(FPM_ADDADVSCRIPTS)
		{
			if(isset($_POST['versionId'])){
				...
				$data['page_sectionid']	= $page_sectionid;
				//$data['pagetype']		= ($pagetype == 1) ? "Section":"Article";
	...
	public function container_lock_status($return_type)
	{
		...
		$current_user_detail  	= $this->template_design_model->get_userdetails_by_id(USERID);
		//$widget_lock_status		= array("msg"=>"","res_status"=>0,"show_msg"=>"0", "passed_params"=>"Instance Id:".$post_instance_id, 'msg_type'=>2);
	...
	{		
		...
		{
			$version_drop_down = '<select name="show_template_version" id="show_template_version" class="" >';
			foreach($template_version_details as $template_version)
			{
				...
				else
				{
					...
					/*elseif($default_version == $version_id && $default_version != '')
					{
						$current_version	= "selected";
					}*/
	...
	/////  Get template published version id( If template published ) or get last version id of current template //////
	/*public function load_published_version_template()
	{
		$page_id			= $this->input->post("page_id");
		$update_page_master = $this->template_design_model->load_published_version_template($page_id);
	}*/
	...
	public function custom_image_processing()
	{
		...
		//$this->template_design_model->custom_image_processing();
	}
	...
	public function get_all_image_temp_table($widget_instance_id, $article_content_id)
	{
		//$this->template_design_model->get_all_image_temp_table($widget_instance_id);
	}
