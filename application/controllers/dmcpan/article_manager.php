...

{
	...
	public function amp_validation(){
		$CI = & get_instance();
		$response = array();
		$response['status'] = 0;
		$response['message']='';
		...
		if($archive!=''){
			$url= AMPURL.$contentID.'/article/'.$timestamp.'/amp_validation.cloud?archive='.$archive;	
		}else{
			$url= AMPURL.$contentID.'/article/'.$timestamp.'/amp_validation.cloud';	
		}
		$response['url'] = $url;
		...
		if(count($result['errors']) > 0){
			for($i=0;$i<count($result['errors']);$i++){
				$response['message'] .='<p>Line no :'.$result['errors'][$i]['line'].' , Message : '.$result['errors'][$i]['error'].' </p>';
			}
		}
		$valid = $response['status'] = ($result['valid']==1) ? 1 : 0;
		$live_db = ($archive!='') ? $CI->load->database('archive_db' , TRUE) : $CI->load->database('live_db' , TRUE);
		if($archive!=''){
			if($live_db->table_exists('article_'.$archive)){
				$live_db->query("UPDATE article_".$archive." SET amp_status='".$valid."' WHERE content_id='".$contentID."'");
			}
		}else{
			$CI->db->query("UPDATE articlemaster SET amp_status='".$valid."' WHERE content_id='".$contentID."'");
			$live_db->query("UPDATE article SET amp_status='".$valid."' WHERE content_id='".$contentID."'");
		}
		...
		if($contentId!=''){
			...
			if($checkId==0){
				...
				if(count($details) > 0){
					switch($type){
						case 'koo' :
							$headers = ['Content-Type:application/json' , 'X-KOO-API-TOKEN:206f0e63-6f70-4e27-98dd-2b8e83b40264'];
