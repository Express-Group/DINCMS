...
if(count($widget_contents)>0)
		...
		if ($original_image_path!='' && getimagesize(destination_base_path . imagelibrary_image_path .$original_image_path))
			{
...
if(count($widget_contents)>0)
		...
		$imagealt ="";
		$imagetitle="";
		if (getimagesize(destination_base_path . imagelibrary_image_path . $Image600X390) && $Image600X390 != '')
		{
		$show_image = image_url. imagelibrary_image_path . $Image600X390;
		}
		else {
		    ...
		    if($original_image_path !='' && getimagesize(image_url . imagelibrary_image_path.$original_image_path))
				{
					...
					if (getimagesize(image_url . imagelibrary_image_path . $Image100X65) && $Image100X65 != '')
					{
	...
	if(count($widget_contents)>0)
 		...
 		foreach($widget_contents as $get_content)
		{
			...
			if($original_image_path!= '' && getimagesize(destination_base_path . imagelibrary_image_path .$original_image_path))
			{
				...
				if (getimagesize(destination_base_path . imagelibrary_image_path . $Image600X300) && $Image600X300 != '')
				{
...
if (getimagesize(destination_base_path . imagelibrary_image_path . $img_path) && $img_path != '')
{
...
echo json_encode($return_value);
  ...
  public function get_image_gallery_list()
  {
	...
	$content_url     = @$gallery_url[0]['url'];
		$param            = $param;
		$live_article_url = base_url().$content_url."?pm=".$param;
		...
		foreach($gallery_details as $gallery_list){
			...
			if(getimagesize(destination_base_path . imagelibrary_image_path. $original_image_path) && $original_image_path != '')
			{			
				$Image600X390   = "";
				$Image600X390 	= str_replace("original","w600X390", $original_image_path);
