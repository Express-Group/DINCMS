...
class Template_designer extends CI_Controller {
	...
	{				
		...
		//$this->memcached_library->flush();  In live comment this line
		...
		if($this->uri->segment(1) == "")
		{
			...
			$home_file           = 'home.php';
			$page                = 'home'; 
			$file                = HOMEURL.'application/views/view_template'.'/'.$home_file;
			$check_file_exist    = file_get_contents($file);
			if($page== "home" && $check_file_exist!= '' && $check_file_exist!=false){  //section
...
echo $this->benchmark->elapsed_time('code_start', 'code_end');exit;
			...
			$viewmode			= "live";
			$allow_here         = true;
		}else if(($this->uri->segment(2)=='rssfeed') && ($this->input->get('id')!=""))
		{
		  $allow_here = false;
		  $section_id = $this->input->get('id');
		  $section_id =  $this->widget_model->get_sectionDetails($section_id, $viewmode);
		   if(count($section_id)>0)
		   {
				if($section_id['ParentSectionID']!=''&& $section_id['ParentSectionID']!=0 ){
				     $parent_section     = $this->widget_model->get_sectionDetails($section_id['ParentSectionID'], $viewmode);
		...
		else 
		{
		  ...
		  else if(substr($this->uri->uri_string(),-3) == 'ece')
		  {
			...
			$year                   = $live_query_string[count($live_query_string)-5];
		    $sectionname             = strtolower($this->uri->segment(1));
			switch ($sectionname) {
				case ($sectionname == "galleries" || $sectionname == "photos"):
					$content_type_id = 3;
	   ...
	   if($allow_here){
		...
		$section_details = $this->widget_model->get_sectionDetails($page_details['menuid'], $viewmode); //live db
