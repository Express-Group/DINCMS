	...
	public function index()
	{
		/* if(! empty($_SERVER['HTTP_USER_AGENT'])){
			...
			if( preg_match('@(iPad|iPod|iPhone|Android|BlackBerry|SymbianOS|SCH-M\d+|Opera Mini|Windows CE|Nokia|SonyEricsson|webOS|PalmOS)@', $useragent) ){
				$AMPURL =base_url().'amp/'.$this->uri->uri_string();
				redirect($AMPURL,'location',301); 
			}
		} */
		$view_mode            = "live";
		...
		if($content_exist && !empty($last_uri))   
		{
			...
			switch ($sectionname) {
				case ($sectionname == "galleries" || $sectionname == "photos" || $sub_section_name=="photo-galleries"):
					$content_type_id = 3;
	  ...
	  else if($ecenic_article)
	  {
		...
		if(count($xml)!= 0)
		{
			...
			$b_section_inc = 0;
			$loop_break_point = ($content_type_id==3 || $content_type_id == 4)? count($template_values_body_content) : count($template_values_body_content);
			for($i=1; $i <= $loop_break_point; $i++){
			
				$body_section 	= $template_values_body_content;
				$section_cl_val	= ($content_type_id==3 || $content_type_id == 4)? $body_section[$b_section_inc] * (12 / array_sum($body_section)) : $body_section[$b_section_inc] * (12 / array_sum($body_section));
