...
function inactive_status(value)
{
	...
	if(r)
	{
		window.location = "<?php echo base_url(); ?>dmcpan/poll_manager/inactivate_poll/"+value;
	}
}
...
function change_status(value, status)
{
	...
	if(r)
	{
		window.location = "<?php echo base_url(); ?>dmcpan/poll_manager/change_status/?id="+value+"&status="+status;
	}
}
...
function delete_poll(value)
{
	...
	if(r)
	{
		window.location = "<?php echo base_url(); ?>dmcpan/poll_manager/delete_data/"+value;
	}		
}
</script>
...
<?php if(defined("USERACCESS_ADD".$Menu_id) && constant("USERACCESS_ADD".$Menu_id) == 1) { ?>
 <p class="FloatRight SaveBackTop"><a href="<?php echo base_url();?>dmcpan/poll_manager/create_poll_page" class="btn-primary btn"><i class="fa fa-file-text-o"></i> &nbsp;Create Poll</a></p>
<?php } ?>
</div>
...
$("#flash_msg_id").slideDown(function() {
	...
	"aoColumnDefs": [
      ...
      $("html, body").animate({ scrollTop: 0 }, "slow");
		...
		"ajax": {
            "url": "<?php echo base_url(); ?>dmcpan/poll_manager/pollmanager_datatable",
			"type" : "POST",
			"data" : {
		...
		else
		{
			...
			switch(pathname)
			{
			 ...
			 $(document.body).on('click', '#status_change', function(event)
			 {
				...
				$.ajax({ 
				url:'<?php echo base_url()."dmcpan/poll_manager/changestatus";?>',
				type:"POST",
