...
<script type="text/javascript" src="<?php echo base_url();?>js/numerology_monthly_predictions.js"></script>
...
{
	$title1 = 'Numerology '.ucfirst($language).' Monthly predictions - Edit';
}else{ 
$title1 = 'Numerology '.ucfirst($language).' Monthly predictions - Create';
}
...
<div class="Container">
	<div class="BodyWhiteBG">
		...
		<?php } ?>
		<form name="monthlyPredictions_form" id="monthlyPredictions_form" action="<?php echo base_url(); ?>dmcpan/numerology_monthly_prediction/add_monthly_predictions" method="post" enctype="multipart/form-data">
			<div class="BodyHeadBg Overflow clear">
				...
				<a class="FloatLeft back-top" href="<?php echo base_url().folder_name; ?>/numerology_monthly_prediction<?php echo $qry;?>"><i class="fa fa-reply fa-2x"></i></a>
...
<div class="rasiqns">
										<label class="question">Month<span id="mandatory" >*</span></label>
...
<div class="rasiqns">
				<label class="question">Year<span id="mandatory">*</span></label>						
										<?php
...
<select name="prediction_date" id="prediction_date" class="controls" onChange="chan(this.value)" >
					 ...
					 echo '<option selected value="'.$fetch_year.'">'.$fetch_year.'</option>'; }
										...
										<p id="date_error1" style="color:#F00"></p>
											...
											<!--<input data-format="dd-MM-yyyy " type="text" name="prediction_date" id="prediction_date" value="<?php /*?><?php if(isset($fetch_values['prediction_date']) && $fetch_values['prediction_date'] != "") { echo $fetch_values['prediction_date'];} ?><?php */?>" />
											<span style="padding-left:20px;"><input type="text" name="prediction_end_date" id="prediction_end_date" value="<?php /*?><?php if(isset($fetch_values['prediction_end_date']) && $fetch_values['prediction_end_date'] != "") { echo $fetch_values['prediction_end_date'];} ?><?php */?>"  readonly />	</span>-->
...
<p id="already_error" style="color:#F00"></p>
									...
									<div class="panchangam-section1">
										<h4 class="ColorTheme">மாதப்பலன்<span id="mandatory">*</span></h4>
	...
	jQuery('<option/>', {
       ...
       /* $(document).ready(function () {
		...
		function updateAb(selected)
		{
			...
			if(date !='')
			{
				$.ajax({
					...
					url: base_url+"dmcpan/numerology_monthly_prediction/change_dateFormat",
		...
		$("#preview_id").click(function() {
			...
			var postdata = {"body_text" :body_text,"date": date,"number":number,"menu_name":'<?php echo 'Numerology Monthly Prediction'; ?>'};
			$.ajax({
