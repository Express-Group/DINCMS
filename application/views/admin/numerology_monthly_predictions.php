...
<div class="breadcrumbs"><a href="<?php echo base_url().folder_name; ?>">Dashboard</a> > <a href="javascript:;"><?php echo $title; ?></a></div>
...
$query = '?lang='.$language; 
$menu_name = 'Monthly Prediction';
}else{
$query = '';
$menu_name = 'Numerology Monthly Tamil';
} ?>
...
<?php if(defined("USERACCESS_ADD".$data['Menu_id']) && constant("USERACCESS_ADD".$data['Menu_id']) == '1') { ?>
 <p class="FloatRight SaveBackTop"><a href="<?php echo base_url().folder_name;?>/numerology_monthly_prediction/add_monthly_predictions<?php echo $query;?>" class="btn-primary btn"><i class="fa fa-file-text-o"></i> &nbsp;Add New</a></p>
<?php } ?>
</div>
...
<div class="FloatLeft w2ui-field">
  <select id="number_id" class="controls">
...
<script>
function get_numerology_predictions() {
	...
	$('#example').dataTable({
		...
		"fnDrawCallback":function(oSettings){
		 $("html, body").animate({ scrollTop: 0 }, "slow");
			...
			if($(this).find('tbody tr').text()== "No matching records found")
			{
				...
				$("#example").find('tbody tr').append($('<td class="BackArrow"><a href="<?php echo base_url(); ?>dmcpan/numerology_monthly_prediction" data-toggle="tooltip" title="Back to list"><i class="fa fa-reply fa-2x"></i></a></td>'));
			}
			else
			{
		...
		"ajax": {
			"url": "<?php echo base_url(); ?>dmcpan/numerology_monthly_prediction/get_numerology_monthlypredictions",
			"type" : "POST",
			"data" : {
			"number_id" : number_id, "from_date" : check_in, "to_date" : check_out, "lang" : "<?php echo $language;?>"}
		}
	});
