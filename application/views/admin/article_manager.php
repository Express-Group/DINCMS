...
.mob_url{float: left;position: absolute;bottom: 0;background: #ffbc00;border: 1px solid #eee;padding: 0 5px 0;color: #fff;font-size: 18px;cursor: pointer;left: 0%;}
/* .pr{position:relative}; */
#example tbody tr td:first-child{position:relative;}
...
<?php } ?>
 <textarea style="display:none;"  id="cpy"></textarea> 
</div>
...
$("#flash_msg_id").slideDown(function() {
	...
	$(document).on( 'click' , '.mob_url'  , function(e){
		toastr.remove();
		var moburl = $(this).parent('p').attr('mobileUrl');
		$('#cpy').html(moburl).show();
		$('#cpy').select();
		document.execCommand("copy");
		$('#cpy').hide();
		toastr.success('Mobile url Copied Successfully');
	});
</script>                       
