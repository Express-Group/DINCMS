...
button.embed,button.embed:hover,button.embed:active,button.embed:focus{background: #f14602   none repeat scroll 0 0 !important;}
.modal{top:47px !important;}
.form-group{width:100%;}
#tbl_name,#tbl_total ,.erp{border: 2px solid #009688 !important;border-radius: 0px;}
.err{color:#F44336;margin-top:2px;}
label{font-weight:700 !important;}
.tbl_box{box-shadow: 0px 1px 4px 0px #00000073;height: 125px;margin-right: 37px; margin-top: 3%;border-radius: 11px;background:#fff;}
...
.tbl_group{margin-top: 11%;text-align: center;margin-left: 12%;}
...
.tbl_del{font-size: 23px;color: red;position: absolute;top: 9px;left: 85%;cursor: pointer;}
.edit-pencil{font-size: 17px;margin-left: 15px;color:#009688;cursor:pointer;}
.tbl_n{width: 56%; float: left; margin: 0;font-size: 13px;}
...
<div class="container" style="margin-top:3%;">
	<div class="row">
		<div class="col-md-12 text-center">
			...
			<textarea id="frames" style="display:none;"></textarea>
			<button class="btn btn-primary margin border-none success" onclick="add_table();">ADD TABLE</button>
			<button class="btn btn-primary margin border-none success" onclick="add_table1();">ELECTION RESULT TABLE</button>
		</div>
	</div>
		...
		<?php
			foreach($table as $Data){
					...
					if($Data->table_properties==null || $Data->table_properties==' '):
						print '<button data-toggle="tooltip" title="Add data" type="button" id="savebtn_'.$Data->tid.'" class="btn btn-primary border-none success" onclick="add_parameter('.$Data->tid.');"><i class="fa fa-plus-circle" aria-hidden="true"></i></button>';
					else:
						print '<button data-toggle="tooltip" title="Add data" type="button" id="savebtn_'.$Data->tid.'" class="btn btn-primary border-none success" onclick="add_parameter('.$Data->tid.');" disabled><i class="fa fa-plus-circle" aria-hidden="true"></i></button>';
					endif;
					if(!empty($Data->table_properties)):
						print '<button data-toggle="tooltip" title="Edit data" type="button"  id="editbtn_'.$Data->tid.'" class="btn btn-primary border-none warn" onclick="edit_parameter('.$Data->tid.')"><i class="fa fa-pencil" aria-hidden="true"></i></button>';
					else:
						print '<button data-toggle="tooltip" title="Edit data" type="button" id="editbtn_'.$Data->tid.'" class="btn btn-primary border-none warn" disabled><i class="fa fa-pencil" aria-hidden="true"></i></button>';
					endif;
					print '<button data-toggle="tooltip" title="Preview" type="button" class="btn btn-primary border-none preview" onclick="preview('.$Data->tid.')"><i class="fa fa-caret-square-o-up" aria-hidden="true"></i></button>';
					print '<button data-toggle="tooltip" title="Embed code" type="button" class="btn btn-primary border-none embed" onclick="embed('.$Data->tid.')"><i class="fa fa-external-link" aria-hidden="true"></i></button>';
					print '</div>';
					print '<span class="tbl_del" onclick="deletes('.$Data->tid.')"><i class="fa fa-times" aria-hidden="true"></i></span>';
					print '</div>';
			}
		?>
	</div>
...
<!---model table starts here-->
  ...
  <!--end-->
  <!--to add table1-->
  <div class="modal fade" id="add-table1" role="dialog">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal">&times;</button>
          <h4 class="modal-title text-center">ELECTION RESULT TABLE</h4>
        </div>
        <div class="modal-body">
			<div class="form-group">
				<label>TABLE TITLE :</label>
				<input type="text" class="form-control erp" id="table_title" style="border: 2px solid #143b5a !important;" value="<?php echo $election_tables['title']; ?>">
			</div>
			<?php
			if($election_tables['list']!=''){
				$list = json_decode($election_tables['list'] , true);
				$template ='';
				if(count($list) > 0 && is_array($list)){
					for($i=0;$i<count($list);$i++){
						$partylist = $list[$i]['list'];
						$template .= '<div class="main-div" style="text-align:center;margin-bottom:15px;"><table class="table party_table" data-title="'.$list[$i]['name'].'">';
						$template .= '<thead>';
						$template .= '<tr>';
						$template .= '<th colspan="4" style="background: #143b5a;color: #fff;text-align:center;">'.$list[$i]['name'].' <i class="fa fa-times remove-tbl" style="color:#fff !important;float: right;cursor:pointer;"></i></th>';
						$template .= '</tr>';
						$template .= '<tr style="background: #6fa6d1;color: #143b5a;font-weight: 700;">';
						$template .= '<th>Party</th>';
						$template .= '<th>Leading</th>';
						$template .= '<th>Won</th>';
						$template .= '<th>Total</th>';
						$template .= '</tr>';
						$template .= '</thead>';
						$template .= '<tbody>';
						for($j=0;$j<count($partylist);$j++){
							$template .= '<tr style="background:#eee;">';
							$template .= '<td><input value="'.$partylist[$j]['party'].'" type="text" style="width:100%;background: #fff !important;"></td>';
							$template .= '<td><input value="'.$partylist[$j]['lead'].'" type="text" style="width:100%;background: #fff !important;"></td>';
							$template .= '<td><input value="'.$partylist[$j]['won'].'" type="text" style="width:100%;background: #fff !important;"></td>';
							$template .= '<td><input value="'.$partylist[$j]['total'].'" type="text" style="width:100%;background: #fff !important;"></td>';
							$template .= '</tr>';	
						}
						$template .= '</tbody>';
						$template .= '</table>';
						$template .= '<button style="background: #143b5a none repeat scroll 0 0 !important;" class="btn btn-primary border-none success add-row">ADD ROW</button>';
						$template .= '<button style="background: #143b5a none repeat scroll 0 0 !important;" class="btn btn-primary border-none success remove-row">REMOVE ROW</button>';
						$template .= '</div>';
					}
				}
				echo $template;
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-primary border-none success" onclick="add_party()" >Add party</button>
          <button type="button" class="btn btn-primary border-none success" onclick="save_party()" >Save</button>
          <button type="button" class="btn btn-primary border-none error" data-dismiss="modal">Exit</button>
        </div>
     </div>
    </div>
  </div>
  ...
  <!--to add table-->
  <div class="modal fade" id="sort_table" role="dialog">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal">&times;</button>
          <h4 class="modal-title text-center">Sort Table</h4>
        </div>
        <div class="modal-body">
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-primary border-none success" onclick="save_sort();" >Save & Exit</button>
          <button type="button" class="btn btn-primary border-none error" data-dismiss="modal">Close</button>
        </div>
     </div>
    </div>
...
function save_party(){
	if($('.main-div').length > 0 && $('#table_title').val().trim()!=''){
		toastr.remove();
		var list =[];
		$('.main-div').each(function(index){
			$(this).find('table').each(function(index1){
				var party_name = $(this).attr('data-title');
				var data = [];
				$($(this).find('tbody')).find('tr').each(function(index2){
					data.push({party : $($(this).find('td').eq(0)).find('input').val() , lead : $($(this).find('td').eq(1)).find('input').val() , won : $($(this).find('td').eq(2)).find('input').val() , total : $($(this).find('td').eq(3)).find('input').val()});
				});
				list.push({name : party_name , list : data});
			});
		});
		var json = JSON.stringify(list);
		$.ajax({
			type:'post',
			url :'<?php print HOMEURL.folder_name?>/dynamic_table/election_table',
			cache:false,
			data :{'title' : $('#table_title').val().trim() , 'list' : json},
		...
		toastr.remove();
		toastr["error"]("Please add table (or) enter valid title");
	}
}
function add_party(){
	var prmpt = prompt("Please enter party name", "");
	if(prmpt!=null && prmpt!='' && prmpt!=undefined){
		$template = '<div class="main-div" style="text-align:center;margin-bottom:15px;"><table class="table party_table" data-title="'+prmpt+'">';
		
		$template += '<thead>';
		$template += '<tr>';
		$template += '<th colspan="4" style="background: #143b5a;color: #fff;text-align:center;">'+prmpt+' <i class="fa fa-times remove-tbl" style="color:#fff !important;float: right;cursor:pointer;"></i></th>';
		$template += '</tr>';
		$template += '<tr style="background: #6fa6d1;color: #143b5a;font-weight: 700;">';
		$template += '<th>Party</th>';
		$template += '<th>Leading</th>';
		...
		$template += '<button style="background: #143b5a none repeat scroll 0 0 !important;" class="btn btn-primary border-none success remove-row">REMOVE ROW</button>';
		$template += '</div>';
		$('#add-table1').find('.modal-body').append($template);
	}
}
function add_table(){
...
function add_table1(){
$("#add-table1").modal({backdrop: 'static',keyboard: false});
}
function save_table(){
	...
	if(tbl_name.trim()==''){
		...
		$.ajax({
			type:'post',
			cache:false,
			url:'<?php print HOMEURL.folder_name?>/dynamic_table/addtable',
			...
			success:function(result){
					...
					template +='<button type="button" data-toggle="tooltip" title="Add Data" id="savebtn_'+result+'" class="btn btn-primary border-none success" onclick="add_parameter('+result+');"><i class="fa fa-plus-circle" aria-hidden="true"></i></button>';
					template +='<button type="button" data-toggle="tooltip" title="Edit Data" id="editbtn_'+result+'" class="btn btn-primary border-none warn" onclick="edit_parameter('+result+')" disabled ><i class="fa fa-pencil" aria-hidden="true"></i></button>';
					template +='<button type="button" data-toggle="tooltip" title="Preview" class="btn btn-primary border-none preview" onclick="preview('+result+')"><i class="fa fa-television" aria-hidden="true"></i></button>';
					template +='<button type="button" data-toggle="tooltip" title="Embed code" class="btn btn-primary border-none embed" onclick="embed('+result+')"><i class="fa fa-external-link" aria-hidden="true"></i></button>';
					template +='</div>';
					template +='<span class="tbl_del" onclick="deletes('+result+')"><i class="fa fa-times" aria-hidden="true"></i></span>';
					template +='</div>';
...
function publish_table(){
	...
	$.ajax({
		type:'post',
			cache:false,
			url:'<?php print HOMEURL.folder_name?>/dynamic_table/Add_Parameter',
...
function preview(tid){
	...
	$.ajax({
		type:'post',
			cache:false,
			url:'<?php print HOMEURL.folder_name?>/dynamic_table/preview',
			data:{'tid':tid},
			success:function(result){
...
function edit_parameter(tid){
	$.ajax({
		type:'post',
			cache:false,
			url:'<?php print HOMEURL.folder_name?>/dynamic_table/edit_table',
			data:{'tid':tid},
			success:function(result){
...
if(t ==true){
	$.ajax({
		type:'post',
		cache:false,
		url:'<?php print HOMEURL.folder_name?>/dynamic_table/delete',
		data:{'tid':tid},
		success:function(result){
			if(result==1){
...
function save_pencil(tid){
	...
	if(tbl.trim()==''){
		...
		type:'post',
		cache:false,
		url:'<?php print HOMEURL.folder_name?>/dynamic_table/inserttablename',
...
function embed(tid){
	var template ='<iframe width="300" height="250" frameborder="0" scrolling="no" src="<?php echo BASEURL ?>embed/table/'+btoa(tid)+'"></iframe>';
...
function sort_table(){
	$('#sort_table').modal({backdrop: 'static',keyboard: false});
}
$(document).ready(function(e){
	$(document).on('click' ,'.add-row' , function(e){
		$template = '<tr style="background:#eee;">';
		$template += '<td><input type="text" style="width:100%;background: #fff !important;"></td>';
		$template += '<td><input type="text" style="width:100%;background: #fff !important;"></td>';
		$template += '<td><input type="text" style="width:100%;background: #fff !important;"></td>';
		$template += '<td><input type="text" style="width:100%;background: #fff !important;"></td>';
		$template += '</tr>';
		$($(this).parent('.main-div').find('table')).find('tbody').append($template);
	});
	$(document).on('click' ,'.remove-row' , function(e){
		var p = $($(this).parent('.main-div').find('table')).find('tbody')
		$(p).find('tr').last().remove();
	});
	$(document).on('click' ,'.remove-tbl' , function(e){
