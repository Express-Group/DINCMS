...
if($subsec_leadstory_remdering_mode == "manual")
{
	...
	//echo '<pre>'; print_r($get_widget_instance_contents); die();	
	...
	if(count($get_widget_instance_contents)>0)
	{
		...
		//$TotalCount = $this->widget_model->get_liveContents_by_sectionId_count_array($max_article_count, $page_section_id, $view_mode, $subsec_leadstory_max_article, $subsec_leadstory_remdering_mode, $multiple_contentID, 3, $is_home);
		 //$TotalCount = count($TotalCount);
		$total_data = $this->widget_model->get_liveContents_by_sectionId_count_array($max_article_count, $page_section_id, $view_mode, $subsec_leadstory_max_article, $subsec_leadstory_remdering_mode, $multiple_contentID, 3, $is_home);
	...
	else
	{
		...
		//$TotalCount = $this->widget_model->get_liveContents_by_sectionId_count_array($max_article_count, $page_section_id, $view_mode, $subsec_leadstory_max_article, $subsec_leadstory_remdering_mode, $leadstory_contentID, 3, $is_home);		
		// $TotalCount = count($TotalCount);
		$total_data = $this->widget_model->get_liveContents_by_sectionId_count_array($max_article_count, $page_section_id, $view_mode, $subsec_leadstory_max_article, $subsec_leadstory_remdering_mode, $multiple_contentID, 3, $is_home);
		$TotalCount = count($total_data);
		/*$last_content_id = @$total_data[$TotalCount-1]['content_id'];
...
else
{
	...
	{
		$get_widget_instance_contents = $this->widget_model->get_all_available_articles_auto($subsec_leadstory_max_article, $page_section_id , 3 ,  $content['mode']);
		if(count($get_widget_instance_contents)>0)
		{
			$leadstory_contentID = array();	
			foreach($get_widget_instance_contents as $leadstory_contentid)
...
if($get_content_ids!='')
{
	...
	foreach($widget_contents as $get_content)
	{
		...
		// Assign summary block starts here	
			...
			$show_simple_tab.= '<div class="play-icon gallery-play-icon"> </div> </figure>';
			...
			if(($TotalCount < $article_count) && ($i == count($widget_contents))  || ($last_content_id == $get_content['content_id']))
			{
				...
				$archieve_count = $this->widget_model->widget_archieve_content_by_section($page_section_id, 'gallery');
