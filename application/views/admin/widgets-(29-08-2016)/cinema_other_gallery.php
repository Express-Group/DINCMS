...
$show_simple_tab       .='<div class="cen-review"><div id="parentHorizontalTab'.$widget_instance_id.'">';
...
foreach($widget_instancemainsection as $get_section)
{
	...
	<div id="slider1_container'.$widget_instance_id.'_'.$j.'" class="jssor-slide cinema-slides-wrapper cinema-slides-right" >';
	$show_simple_tab .='<div u="slides" class="cinema-slides" id="cinema_slides_id_'.$widget_instance_id.'_'.$j.'">';

	$gallery_id      = @$widget_instance_contents[0]['content_id'];	
	...
	foreach($gallery_details as $gallery_list){
		...
		if(getimagesize(image_url . imagelibrary_image_path. $original_image_path) && $original_image_path != '')
		{			
			$Image600X390   = "";
...
if(count($widget_contents)>0)
{
	...
	{	
		...
		if(($m == 0) || ($m == count($widget_instance_contents))){
...
<script type="text/javascript">
	...
	//$('.photo-sec-next').slick('setPosition');	
	//$('#singleplay6_<?php echo $widget_instance_id; ?>').slick('setPosition');	
...
$(document).ready(function(){	
	...
	$("div.img").click(function(e){
		...
		{ 
			...
			type: "POST",
			data: {"content_id":content_id, "view_mode":'<?php echo $view_mode;?>', "param": '<?php echo $content['page_param'];?>',},
			dataType : "html",
			async : false	
