...
//echo '<pre>'; print_r($subsection_leadstory); die();
...
<div class="Other-Story section-border">';
...
$archive =  '';
$manual_instance = "&widget2_instance=archive";
...
if($subsec_leadstory_remdering_mode == "manual")
{
	...
	if(count($get_widget_instance_contents)>0)
	{
		...
		//$TotalCount = $this->widget_model->get_liveContents_by_sectionId_count_array($max_article_count, $page_section_id, $view_mode, $subsec_leadstory_max_article, $subsec_leadstory_remdering_mode, $multiple_contentID, 4, $is_home);
		 
		$total_data = $this->widget_model->get_liveContents_by_sectionId_count_array($max_article_count, $page_section_id, $view_mode, $subsec_leadstory_max_article, $subsec_leadstory_remdering_mode, $multiple_contentID, 4, $is_home); 
	...
	else
	{
		...
		//$TotalCount = $this->widget_model->get_liveContents_by_sectionId_count_array($max_article_count, $page_section_id, $view_mode, $subsec_leadstory_max_article, $subsec_leadstory_remdering_mode, $leadstory_contentID, 4, $is_home);
...
else
{
	...
	{
		$get_widget_instance_contents = $this->widget_model->get_all_available_articles_auto($subsec_leadstory_max_article, $page_section_id , 4 ,  $content['mode']);
		if(count($get_widget_instance_contents)>0)
		{
			$leadstory_contentID = array();	
			foreach($get_widget_instance_contents as $leadstory_contentid)
...
if($get_content_ids!='')
{
	...
	$widget_instance_contents1 = $this->widget_model->get_contentdetails_from_database($get_content_ids, 4, $is_home, $view_mode);	
	...
	foreach($widget_contents as $get_content)
	{
		...
		// Assign summary block starts here	
			...
			$show_simple_tab.= '<div class="play-icon"> </div> </figure>';
			...
			if(($TotalCount < $article_count) && ($i == count($widget_contents))  || ($last_content_id == $get_content['content_id']))
			{
				...
				$archieve_count = $this->widget_model->widget_archieve_content_by_section($page_section_id, 'video');
...
echo $PaginationLink ;
echo $archive.'</div><div class="common-border"><span></span><span></span></div></div></div>';
?>
