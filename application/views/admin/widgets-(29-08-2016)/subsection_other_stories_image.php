...
{
	$show_simple_tab.=	'<h4 class="other_story_heading">தற்போதைய செய்திகள்</h4>';
}
else
{
$show_simple_tab.=	'<h4 class="other_story_heading">'.$widget_custom_title.'</h4>';
}

//$show_simple_tab.= ' <div class="btn-group"><a href="#" id="list_'.$widget_instance_id.'" class="btn btn-default btn-sm"><span class="fa fa-th-list"></span>List</a><a href="#" id="grid_'.$widget_instance_id.'" class="btn btn-default btn-sm"><span class="fa fa-th-large"></span>Grid</a></div>';
...
elseif($subsec_leadstory_remdering_mode == "auto" && $subsec_leadstory_sectionID === $content['sectionID'])
{
	...
	//$subsec_leadstory_remdering_mode = 'manual';
}

$article_limit = ($this->input->get('per_page') != '') ? $this->input->get('per_page'):0;

$widget_instance_contents = $this->widget_model->get_liveContents_by_sectionId_count_array($max_article_count, $page_section_id, $view_mode, $subsec_leadstory_max_article, $subsec_leadstory_remdering_mode, $multiple_contentID, 1, $is_home);
...
if($get_content_ids!='')
{
	...
	$widget_instance_contents1 = $this->widget_model->get_contentdetails_from_database_per_page($get_content_ids, $content_type, $is_home, $view_mode, $article_limit,$article_count);
	...
	foreach($widget_contents as $key_value => $get_content)
	{
		...
		if($count <= 3)
		{
		   ...
		   $show_simple_tab.= '<div class="row">'; 
				...
				$show_simple_tab.= '<div class="item  col-lg-4 col-md-4 col-sm-4 col-xs-12 list-group-item"><div class="thumbnail"> ';

				//$show_simple_tab.= '<a  href="'.$live_article_url.'" class="article_click" ><img class="group list-group-image" src="'.$dummy_image.'" data-src="'.$show_image.'" title = "'.$imagetitle.'" alt = "'.$imagealt.'"></a>';
				$show_simple_tab.= '<div class="">
					<h5 class="group inner list-group-item-heading sub-grid-des">'. $display_title.'</h5>
					 <p class="group inner list-group-item-text summary">'.$custom_summary.'</p>';
					 //$time= $lastpublishedon; 
				 //$post_time= $this->comment_model->time2string($time);
				//$show_simple_tab.=	'<p class="post_time">'.$post_time.' ago</p>';
				
				$show_simple_tab.=	'<p class="post_time">'.date('d-m-Y',strtotime($lastpublishedon)).'</p>'; 
			...
			if(($TotalCount < $article_count) && ($i == count($widget_contents))  || ($last_content_id == $get_content['content_id']))
			{
				...
				$archieve_count = $this->widget_model->widget_archieve_content_by_section($page_section_id, 'article');
...
$show_simple_tab .=$archive.'</div><div class="common-border"><span></span><span></span></div></div></div>';
