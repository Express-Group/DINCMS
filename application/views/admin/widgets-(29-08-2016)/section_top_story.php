...
if($content['RenderingMode'] == "manual")
{
	...
	if(count($widget_instance_contents)>0)
	{
		...
		if($content_type!=1)
		{
			...
			//else
				//$content_details = $this->widget_model->widget_article_admin_preview($content['sectionID'], $content_type);
		}
	}
}
else
{
	...
	if (stripos(strtolower($section_details['URLSectionStructure']), 'galleries') !== false)
	{
		...
		//else
			//$gallery_id = $this->widget_model->widget_article_admin_preview($content['sectionID'], $content_type);
...
$live_article_url = $article_url."?pm=".$param;
 ...
 <h1 class="ArticleHead" id="content_head" itemprop="name"><a href=<?php echo $live_article_url ?> class="article_click"><?php echo $content_title;?></a></h1>
  <div class="slide GalleryDetail section_top_gallery GalleryDetailSlide" style="width:100% !important">
	...
	foreach($get_gallery_images as $gallery_image){ 
				  ...
				  if (getimagesize(image_url . imagelibrary_image_path . $image_path) && $image_path != '')
					{
				  $imagedetails = getimagesize(image_url . imagelibrary_image_path. $image_path);
