...
<script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js"></script>
  <style>
	...
	.form-group input[type="text"] , .form-group input[type="file"]{border:none;border-radius:0;border: 2px solid #3c8dbc !important;width:100%;}
	...
	.BodyHeadBg{position:relative;top: 53px;}
...
<div class="Container">
	<div class="BodyHeadBg Overflow clear">
		<div class="FloatLeft  BreadCrumbsWrapper PollResult">
			<div class="breadcrumbs">Dashboard > Highlights Manager</div>
			<h2>Highlights News Manager</h2>
		</div>
		<p class="FloatRight SaveBackTop article_save">
			<a href="javascript:add_scroll_news();" class="btn-primary btn"><i class="fa fa-file-text-o"></i> &nbsp;Add Highlights</a>
		</p>
		<table class="table table-bordered" id="table-jquery">
			...
			print '<tr class="scroll_'.$data->hid.' text-center">';
			print '<td>'.$i.'</td>';
			print '<td class="content_'.$data->hid.'">'.$data->content.'</td>';
			print '<td><button class="btn btn-primary" onclick="edit_news('.$data->hid.');" data-toggle="tooltip" data-placement="right" title="edit"><i class="fa fa-pencil" aria-hidden="true"></i></button></td>';
			print '<td><button class="btn btn-primary error" onclick="delete_news('.$data->hid.');" data-toggle="tooltip" data-placement="right" title="delete"><i class="fa fa-trash-o" aria-hidden="true"></i></button></td>';
			print '</tr>';
			$i++;
			}
			?>
			...
			<div class="modal-dialog">
			  <div class="modal-content">
				...
				<div class="modal-body">
					<form method="post" action="<?php echo base_url(folder_name.'/scrolling_news/save_highlights') ?>" id="form-s" enctype="multipart/form-data">
	  ...
	  <div class="modal fade" id="edit_news" role="dialog">
			<div class="modal-dialog">
			  <div class="modal-content">
				<div class="modal-header">
				  <h4 class="modal-title">Edit Scrolling News</h4>
				</div>
				<div class="modal-body">
					<form method="post" action="<?php echo base_url(folder_name.'/scrolling_news/save_edit_highlights') ?>" id="form-e" enctype="multipart/form-data">
					<div class="form-group">
						<label>Enter Content:</label>
						<!--<input type="text" name="news" id="news" class="">-->
						<textarea class="ckeditor" id="scroll_text1"  name="scroll_text1"></textarea>
						<input style="display:none;" type="text" id="edit_new_id" name="edit_new_id">
					</div>
					</form>
				</div>
				<div class="modal-footer">
				  <button type="button" class="btn btn-primary success main-btn" onclick="esave_news();"><i class="fa fa-floppy-o" aria-hidden="true"></i></button>
				  <button type="button" class="btn btn-primary error" data-dismiss="modal"><i class="fa fa-times-circle" aria-hidden="true"></i></button>
				</div>
			  </div>
			</div>
...
function esave_news(){
	var news=CKEDITOR.instances.scroll_text1.getData();
	var headline = $('#headline1').val();
	//var news=$('#news').val();
		if($(news).text().trim() ==''){
			//$('#news_error').show().html('Please fill the required fields');
			toastr["error"]("Please fill the required fields");
...
function edit_news(sid){
	$.ajax({
		type: "POST",
		cache:false,
		url: "<?php echo base_url().folder_name ?>/scrolling_news/fetch_highlights/",
		data:{'sid':sid},
		dataType:'json',
		success: function(result){
			if(result.response==1){
				CKEDITOR.instances['scroll_text1'].setData(result.content);
				$('#edit_new_id').val(sid);
				$("#edit_news").modal({backdrop: 'static', keyboard: false});
...
function del_news(sid){
	var ajax=$.ajax({
		type:'post',
		cache:false,
		url:'<?php print base_url().folder_name?>/scrolling_news/delete_highlights/',
		data:{'sid':sid}
		});
		...
		if(result==1){
			...
			toastr["success"]("Hightlights News deleted Successfully");
...
$(document).ready(function(){
		var config={		
			...
			extraPlugins: 'autogrow,html5audio,embed,colordialog,fontAwesome,notification,simage', 
  		...
  		CKEDITOR.replace('scroll_text1',config );
