...
<body>
							...
							if($this->uri->segment(4)==''):
								...
								foreach($data as $article_row){
									...
									if($notification_status==0):
										...
										?>
										<button class="btn btn-primary b-<?=$article_row->content_id ?>" title="set notification" onclick="set_notification('<?=$article_row->content_id ?>');"><i class="fa fa-bell-o" aria-hidden="true"></i></button>
										...
										print '<td class="text-center">';
										?>
										<button class="btn btn-danger b-<?=$article_row->content_id ?>" title="notified" onclick="set_notification('<?=$article_row->content_id ?>');" disabled><i class="fa fa-lock" aria-hidden="true"></i></button>
										<?php
										print '</td>';
	...
	function set_notification(content_id){
		...
		if(app==true){
			...
			$.ajax({
				...
				url:'<?php print site_url()?>'+'dmcpan/Push_notification/send_notification',
	...
	function share(type,id){
		...
		if(type==1){
			...
			url_content = encodeURI(url_content);
			var fb_url='https://twitter.com/intent/tweet?original_referer='+url_content+'&text='+title+'&url='+url_content+'&via=Dinamani';
			...
			success:function(result){
				...
				var fb_url='https://twitter.com/intent/tweet?original_referer='+result.id+'&text='+title+'&url='+result.id+'&via=Dinamani';
				window.open(fb_url, "", "width=800,height=400");
			}
			}); */
		}	
