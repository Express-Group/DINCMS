...
<div class="Container">
	<div class="BodyWhiteBG">
		<div class="BodyHeadBg Overflow clear">
            ...
            <?php $data['Menu_id'] = get_menu_details_by_menu_name("Role");
		if(defined("USERACCESS_ADD".$data['Menu_id']) && constant("USERACCESS_ADD". $data['Menu_id']) == 1) 
		{ ?>
             <p class="FloatRight SaveBackTop remoda1-bg"><a href="<?php echo base_url();?>dmcpan/rolemaster_manager/role_form_view" type="button" class="btn-primary btn"><i class="fa fa-plus-circle"></i>Add New</a></p>
             <?php }?>
...
<script>
	...
	function delete_rolemaster(id)
		{
			var r = confirm("Are you sure you want to delete?");
			if(r==true){
			window.location.href="<?php echo base_url() ?>dmcpan/rolemaster_manager/rolemaster_delete/"+id;
		}
	}
</script>
<script>
	...
	$('#example').dataTable( 
{
    ...
    $('#example').dataTable( {
      ...
      if ($(this).find('tbody tr').length<=1) {
		...
		"fnDrawCallback":function(oSettings )
		{
		   ...
		   if($(this).find('tbody tr').text()== "No matching records found")
		   {
			  ...
			  $("#example_length").hide();
			  // $("#example").find('tbody tr').append($('<td class="BackArrow"><a href="<?php  echo base_url(); ?>dmcpan/rolemaster_manager" data-toggle="tooltip" title="Back to list"><i class="fa fa-reply fa-2x"></i></a></td>'));
	 ...
	 $("#example").find('tbody tr').html($('<td valign="top" colspan="10" class="dataTables_empty BackArrow">No matching records found <a href="" data-toggle="tooltip" title="Back to list"><i class="fa fa-reply fa-2x"></i></a></td>'));
		...
		"ajax": {
            "url": "<?php echo base_url(); ?>dmcpan/rolemaster_manager/roledepartment_datatable",
			"type" : "POST",
			"data" : {
