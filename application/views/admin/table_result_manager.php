...
<div class="Container">
	<div class="BodyWhiteBG">
		<div class="BodyHeadBg Overflow clear">
          ...
          <?php $data['Menu_id'] = get_menu_details_by_menu_name('Table Result');
		if(defined("USERACCESS_ADD".$data['Menu_id']) && constant("USERACCESS_ADD".$data['Menu_id']) == 1) 
		{ ?>
             <p class="FloatRight SaveBackTop remoda1-bg"><a href="<?php echo base_url();?>dmcpan/table_result_manager/table_result_table_view" type="button" class="btn-primary btn"><i class="fa fa-plus-circle"></i>Add New</a></p>
			 <?php }?>
...
{ 
$success_status = substr_count($_GET['status'] , '_success');
if($success_status==1){
$status= ($_GET['status']=='add_success')? 'Table data added Successfully!'  : 'Table data updated Successfully!' ;  
$class = "SessionSuccess";
}else{
$status= ($_GET['status']=='add_error')? 'Problem while inserting. Please try again!'  : 'Problem while updating. Please try again!' ;  
$class = "SessionError";
}
?>
 <div class="FloatLeft <?php echo $class;?>" id="flash_msg_id"><?php echo $status;?></div>
<?php
}
?> 
<?php 
	...
	function delete_topic(id)
		{
			...
			if(r==true){
			window.location.href="<?php echo base_url() ?>dmcpan/table_result_manager/table_result_delete/"+id;
		}
	}
</script>
<script>
	...
	$('#example').dataTable( 
{
    ...
    .append($('<td class="BackArrow"><a href="<?php  echo base_url(); ?>dmcpan/table_result_manager" data-toggle="tooltip" title="Back to list"><i class="fa fa-reply fa-2x"></i></a></td>'));
		   }
		   else
		   {
		...
		"ajax": {
            "url": "<?php echo base_url(); ?>dmcpan/table_result_manager/table_result_datatable",
			"type" : "POST",
			"data" : {
...
$("#flash_msg_id").slideDown(function() {
	...
	window.history.pushState('', '', "<?php echo base_url() ?>dmcpan/table_result_manager/");
});
<?php } ?>
