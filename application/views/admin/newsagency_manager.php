	...
	<div class="BodyWhiteBG">
		<div class="BodyHeadBg Overflow clear">
			<div class="FloatLeft BreadCrumbsWrapper ">
				<div class="breadcrumbs">Dashboard>News Agency</div>
 					<h2>News Agency Manager</h2>
			</div>
			 <?php $data['Menu_id'] = get_menu_details_by_menu_name("News Agency");
		if(defined("USERACCESS_ADD".$data['Menu_id']) && constant("USERACCESS_ADD". $data['Menu_id']) == 1) 
		{ ?>
             <p class="FloatRight SaveBackTop remoda1-bg"><a href="<?php echo base_url();?>dmcpan/newsagency_manager/newsagency_form_view" type="button" class="btn-primary btn"><i class="fa fa-plus-circle"></i>Add New</a></p>
			 <?php }?>
...
<!--<i  id="newsagency_search" class="fa fa-search FloatLeft" ></i>
<button class="btn-primary btn margin-left-50" type="button" id="clear_search">Clear Search</button>-->

<button class="btn btn-primary" type="button" id="newsagency_search">Search</button>
...
<script>
	...
	function delete_newsagency(id)
		{
			var r = confirm("Are you sure you want to delete?" );
			if(r==true){
			window.location.href="<?php echo base_url() ?>dmcpan/newsagency_manager/newsagency_delete/"+id;
		}
	}
</script>
<script>
	...
	$('#example').dataTable( 
{
    ...
    $('#example').dataTable( {
      ...
      if ($(this).find('tbody tr').length<=1) {
		...
		"ajax": {
            "url": "<?php echo base_url(); ?>dmcpan/newsagency_manager/newsagency_datatable",
			"type" : "POST",
			"data" : {
...
$(document).ready(function()
{
		...
		$('body').keypress(function (e) {
			...
			if(e.which == 13) {
				$("#newsagency_search").click();
			}
			
		});
	
	
	$("#newsagency_search").click(function()
 	{
	// console.log('ppp');
