...
<div class="Container">
	<div class="BodyWhiteBG">
		<div class="BodyHeadBg Overflow clear">
			...
			<a href="<?php echo base_url();?>dmcpan/breaking_news_manager/create_breaking_news" class="btn-primary btn"><i class="fa fa-file-text-o"></i> &nbsp;Add Breaking News</a>
			<?php } ?>
				...
				<div class="row AskPrabuCheckBoxWrapper">
					<ul class="AskPrabuCheckBox FloatLeft">
						...
						<tr>
							...
							<td>
								<input class="BorderBoxNone" id="price" type="range" min="3" max="5" value="<?php if(isset($scroll_speed['breakingNews_scrollSpeed']) && $scroll_speed['breakingNews_scrollSpeed'] != "") { echo $scroll_speed['breakingNews_scrollSpeed']; } ?>" />
							</td>
							<td>
...
$("#btnScrollSpeed").click(function()
{
	...
	var scroll_txt = $("#result").html();
		...
		{
			$.ajax({
					url:"<?php echo base_url(); ?>dmcpan/breaking_news_manager/news_scroll_speed", // Url to which the request is send
...
$("#news_publish").click(function()
{
	...
	{
		$.ajax({
				url:"<?php echo base_url(); ?>dmcpan/breaking_news_manager/publish_breakingnews", // Url to which the request is send
				type: "POST",             // Type of request to be send, called as method
				data:  { },
				success: function(data)   // A function to be called if request succeeds
				{
	...
	"aoColumnDefs": [
	 ...
	 //"order": [1,"asc"],
      ...
      $("html, body").animate({ scrollTop: 0 }, "slow");
		   ...
		   if($(this).find('tbody tr').text()== "No matching records found")
			{
			 ...
			 $('.dataTables_info').hide();
			  $("#example").find('tbody tr').append($('<td class="BackArrow"><a href="<?php echo base_url(); ?>dmcpan/breaking_news_manager" data-toggle="tooltip" title="Back to list"><i class="fa fa-reply fa-2x"></i></a></td>'));
			 }
			 else
			 {
		...
		"ajax": {
            "url": "<?php echo base_url(); ?>dmcpan/breaking_news_manager/breakingNews_datatable",
			"type" : "POST",
			"data" : {
		...
		switch(set_status)
		{
			...
			if(set_status)
			{
				if(status_alert == true)
				{
					...
					$("#commom_loading").show();
							$.ajax({ 
								url:'<?php echo base_url()."dmcpan/breaking_news_manager/changestatus";?>',
								type:"POST",
...
function delete_news_func(value)
{
	...
	if(r)
	{
		window.location = "<?php echo base_url(); ?>dmcpan/breaking_news_manager/delete_data/"+value;
	}		
}	
...
function update_disp_order()
{
	...
	$.ajax({
		url: "<?php echo base_url(); ?>dmcpan/breaking_news_manager/update_dispOrder",
		type: "POST",
