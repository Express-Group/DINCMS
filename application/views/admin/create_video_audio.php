...
<form  name="video_audio_upload" id="video_audio_upload" action="<?php echo site_url();?>dmcpan/audio_video_manager/insert_update" method="post"  novalidate="novalidate"  enctype="multipart/form-data" >
...
<?php if(isset($get_content_id) && $get_content_id != "") { ?>
		...
		var postdata = "get_content_id="+<?php echo $get_content_id ?>+"&contentversion_id="+<?php echo $contentversion_id ?>+"&print_status="+print_status+"&publishdate="+publish_date+"&unpublishdate="+"<?php echo $unpublishdate ?>"+"&content_type="+<?php echo $check_content_type ?>;
			...
			$.ajax({
					url: base_url+"dmcpan/common/status_statement", // Url to which the request is send
...
<p class="FloatRight save-back save_margin article_save">
 <a class="back-top FloatLeft" href="<?php echo site_url();?>dmcpan/<?php echo $page_redirect; ?>" title="Go Back"><i class="fa fa-reply fa-2x"></i></a>
...
<script type="text/javascript">
	...
	$(function() {
		$( "#tagform-full" ).find('input.tag').tagedit({
			autocompleteURL: "<?php echo base_url(); ?>dmcpan/common/tags",
	...
	function add_byliner(name)
	{
		...
		$.ajax({
				url: base_url+"dmcpan/audio_video_manager/add_txtbyliner", // Url to which the request is send
				type: "POST",             // Type of request to be send, called as method
