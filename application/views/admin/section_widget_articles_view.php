...
{
	var dataTableAjaxUrl = base_url + "dmcpan/section_widget_articles/get_widgetInstance_related_article";
}
else
{
	var dataTableAjaxUrl = base_url + "dmcpan/section_widget_articles/get_widgetInstancearticle";
}
	...
	table = $('#example').DataTable( {
        ...
        "ajax": { 
					...
					"data" 		: function(d) {																		
											...
											//$('#hidden_parent_section_id').val($('#search_bysection').find('option:selected').attr('parent_sec_id'););
    ...
    $('#example tbody').on('click', '.widget_toggle', function () {
		...
		$(".save_custom_details").on("click", function (e) {
			...
			//if(!remove_image)
			{
					...
					if( $("#customImage_" + content_id + "_view").attr('src') != '' )
					{
						...
						$.ajax({
							url: base_url+"dmcpan/section_widget_articles/check_custom_image_name", // Url to which the request is send
		...
		function check_image_name_exists()
			...
			postdata = "physical_name="+physical_name+'.'+$("#physical_name"+get_content_id).attr('physical_extension')+"&temp_id="+$("#physical_name"+get_content_id).attr('rel');
				$.ajax({
					url: base_url+"dmcpan/common/check_image_name", // Url to which the request is send
					type: "POST",             // Type of request to be send, called as method
					data:  postdata,
					dataType: "json",
					async: false, 
...
function savePermanent(istemporary)
{
	...
	{ 		
		...
		{
			save_url = base_url + "dmcpan/section_widget_articles/add_widget_article/saveTemporary";
		}
		else if(istemporary == 'saveTemporary-savepermanent-publish_articles')
		{
			save_url = base_url + "dmcpan/section_widget_articles/add_widget_article/publish/s";
		...
		{
			save_url = base_url + "dmcpan/section_widget_articles/add_widget_article/saveTemporary-savepermanent/s";
...
<div class="Container">
	<div class="BodyWhiteBG">
		<div class="BodyHeadBg Overflow clear">
			...
			<a class="back-top FloatRight" target="_blank" href="<?php echo base_url(); ?>dmcpan/template_designer/load_saved_template/<?php echo $homepage_id['Page_master_id'].'/'.$homepage_id['Version_Id']; ?>" title="Preview"><i class="fa fa-desktop i_extra"></i></a>
			<?php } ?>
		...
		<div class="Overflow DropDownWrapper">
			<div class="previewcontainer wid-main">
				<form name="article_popup_form" id="article_popup_form" action="<?php echo base_url()."dmcpan/section_widget_articles/add_widget_article/saveTemporary"; ?>" method="post" enctype="multipart/form-data">
                  ...
                  <select class="controls" name="search_bytype" id="search_bytype" >
					...
					?>                    
							...
							<div class="FloatLeft w2ui-field" <?php if($check_widget_type == "jothida_manager") {  ?> style="display:none" <?php } ?>> 
								...
								<select class="controls" name="search_bysection" id="search_bysection" <?php if($check_widget_type == "jumbo_widget_articles"  || $check_widget_type == "related_articles") {  ?> onchange="search_articles();" <?php } ?> >
									...
									<?php if(isset($section_mapping)) { 
										foreach($section_mapping as $mapping) {  
											...
											if( $mapping['Sectionname'] != 'முகப்பு' )										
											{ 
											
											if( $mapping['Sectionname'] == 'ஜோதிடம்' && $check_widget_type == 'jothida_manager' or ($mapping['Sectionname']!= 'புகைப்படங்கள்' && $mapping['Sectionname']!= 'வீடியோக்கள்' && $mapping['Sectionname']!= 'ஆடியோக்கள்' && $check_widget_type != 'jumbo_widget_articles' && $check_widget_type != 'jothida_manager' ) or $check_widget_type == 'jumbo_widget_articles'  or $check_widget_type == 'related_articles')										
											{ 
									?>
...
$(document).ready(function(){
	...
	//var hidden_parent_section_id = $('#hidden_parent_section_id').val();
	...
	//	alert(hidden_parent_section_id);
		temp_save_articles();
	});
...
function temp_save_articles(show_temp)
{
   ...
   newcheckbox_checked = $('input[type="checkbox"][name^=articles_list]:checked',rowhtml);
	...
	{ 
		$.ajax({
			url			: base_url+"dmcpan/section_widget_articles/view_temparticles",
			method		: 'post',
			async		: async_status,
...
{		
	$.ajax({
			url		: base_url+"dmcpan/section_widget_articles/Insert_temp_from_image_library",
			type	: "POST",
			data	: ImageData,
			dataType: "json",
			async	: false, 	
	...
	<div class="article_popup GalleryPopup ArticlePopup" style="height: 467px;">
		...
		<div class="article_popup2">
			<div class="article_upload">
				<form  name="ImageForm" id="ImageForm" action="<?php echo base_url(); ?>dmcpan/section_widget_articles/custom_image_upload" method="POST" enctype="multipart/form-data">
			...
			<div class="article_browse1">
				...
				<?php if(isset($image_library)) { 
						 ...
						 else
						 {
						  ...
						  ?>
				<nav id="page-nav"> <a href="<?php echo base_url(); ?>dmcpan/section_widget_articles/search_image_library_scroll/2"></a> </nav>
			</div>
	...
	jqis(".set_image").click(function(){
	 ...
	 var $container = jqis('.popup_images');
		...
		$container.html('<div id="LoadingSelectImageLocal" style="display: block;"><img src="<?php echo base_url(); ?>images/admin/loadingimage.gif" style="border:none; width:23px; height:23px;text-align: center;float: none;margin: 0px;padding: 0px;"><br>Loading ...</div>');
				...
				$.ajax({
				url: base_url+"dmcpan/section_widget_articles/get_image_library_scroll/1", // Url to which the request is send
				type: "POST",             // Type of request to be send, called as method
				dataType: "HTML",
...
$container.html('<div id="LoadingSelectImageLocal" style="display: block;"><img src="<?php echo base_url(); ?>images/admin/loadingimage.gif" style="border:none; width:23px; height:23px;text-align: center;float: none;margin: 0px;padding: 0px;"><br>Loading ...</div>');
	...
	jqis.ajax({
		url: base_url+"dmcpan/section_widget_articles/search_image_library",
		type: "POST",
		data: postdata,
		dataType: "json",
		success: function(data){
...
{	
   ...
   if(confirm(confirm_text))
	  {	
		...
		$.ajax({
				url: base_url + "dmcpan/section_widget_articles/widgetarticle_customdetails",
				type: 'post',
				processData: false,
				contentType: false,
				async: false,
