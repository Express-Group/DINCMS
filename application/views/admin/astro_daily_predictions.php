...
<?php if(defined("USERACCESS_ADD".$data['Menu_id']) && constant("USERACCESS_ADD".$data['Menu_id']) == '1') { ?>
 <p class="FloatRight SaveBackTop"><a href="<?php echo base_url();?>dmcpan/astro_daily_prediction/add_daily_predictions" class="btn-primary btn"><i class="fa fa-file-text-o"></i> &nbsp;Add New</a></p>
<?php } ?>
</div>
...
function get_astro_predictions() {
	...
	$('#example').dataTable({
		...
		"fnDrawCallback":function(oSettings){
		   $("html, body").animate({ scrollTop: 0 }, "slow");
			...
			if($(this).find('tbody tr').text()== "No matching records found")
			{
				...
				$("#example").find('tbody tr').append($('<td class="BackArrow"><a href="<?php echo base_url(); ?>dmcpan/astro_daily_prediction" data-toggle="tooltip" title="Back to list"><i class="fa fa-reply fa-2x"></i></a></td>'));
			}
			else
			{
		...
		"ajax": {
			"url": "<?php echo base_url(); ?>dmcpan/astro_daily_prediction/get_astro_dailypredictions",
			"type" : "POST",
			"data" : {
