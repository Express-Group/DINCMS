...
echo $this->session->flashdata('msg');
   ...
   <form action="<?php echo base_url(); ?>dmcpan/article_manager/update_article/<?php echo $get_article_details['content_id']; ?>" method="post" id="content_form" name="content_form" enctype="multipart/form-data">
...
$("#show_the_map_id").change(function(event) {
	...
	$("#search_months").change(function() {
			...
			$.ajax({
				url: "<?php echo base_url(); ?>dmcpan/article_image/search_image_library",
				type: "POST",
				data: postdata,
				dataType: "json",
	...
	var postdata = "type="+Type+"&section="+Section+"&search_text="+search_text+"&check_in="+check_in+"&check_out="+check_out;
		...
		$.ajax({
			url: base_url+"dmcpan/article/search_internal_article", // Url to which the request is send
			type: "POST",             // Type of request to be send, called as method
			data:  postdata,
			dataType: "json",
			success: function(data)   // A function to be called if request succeeds
...
function uploadFormData(formData) {
	...
	if($.trim($("#alt_tag").val()) != '' &&  $.trim($("#caption").val()) != '' && $("#country_id").val() != '' && $("#state_id").val() != '' && $("#city_id").val() != '') {
		...
		$.ajax({
			url: "<?php echo base_url(); ?>dmcpan/article_image/image_upload",
			type: "POST",
			data: formData,
			contentType:false,
			cache: false,
