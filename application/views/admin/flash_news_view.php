...
<script src="<?php echo image_url ?>js/toastr.js"></script>
  ...
  <style>
	...
	#save,#edit_save,#publish,#reset,#move{width:auto !important;color:#fff !important;}
	#breaking_news option {padding: 10px;border-bottom: 1px solid #eeeeee;}
	...
	<div class="container-fluid">
		...
		<div class="row">
			<div class="col-md-6 col-lg-6" style="border-right: 2px solid #e3e4e3;">
				...
				$breakingNews = $this->live_db->query("SELECT Title FROM breakingnewsmaster")->result();
				echo '<div class="form-group" style="width:100%;">';
				echo '<label for="breaking news" style="font-size: 18px;font-weight: bold !important;">Select breaking news: </label>';
				echo '<select class="form-control" id="breaking_news" multiple style="border: 2px solid #c8c8c8 !important;background-color: #fff !important;">';
				foreach($breakingNews as $news):
					echo '<option value="'.strip_tags($news->Title).'">'.strip_tags($news->Title).'</option>';
				endforeach;		
				echo '</select>';
				echo '</div>';
				...
				<div class="form-group pull-right" style="width:100%;text-align:right;">
					<input type="button" value="move" id="move" class="btn btn-success">
				</div>
			...
			<div class="col-md-6 col-lg-6 flashnews_content">
				...
				if($contents!='EMPTY'):
					...
					for($i=0;$i<count($contents);$i++):
						...
						echo '<div class="content" contentid="'.$j.'" style="width:100%;float:left;">';
						echo $contents[$i];
						echo '</div>';
...
$(document).ready(function(){
	...
	$('#move').on('click',function(){
		var breaking_news = $('#breaking_news').val();
		if(breaking_news.length!=0){
			for($i=0;$i<breaking_news.length;$i++){
				var template ='';
				template +='<div class="flash-content">';
				template +='<div class="content" contentid="'+$('.content').length+'" style="width:100%;float:left;">';
				template +=breaking_news[$i];
				template +='</div>';
				template +='<div style="width:100%;float:left;text-align:right;padding-top: 1%;    border-top: 1px solid #dedbdb;">';
				template +='<a class="edit" style="padding-right: 8px;color:blue;cursor:pointer;text-decoration:none;"><i class="fa fa-pencil" aria-hidden="true"></i></a>';
				template +='<a class="delete"  style="color:blue;cursor:pointer;text-decoration:none;"><i class="fa fa-trash-o" aria-hidden="true"></i></a>';
			...
			toastr.success("News addedsuccessfully");
		}else{
			toastr.error("Please select one or more news");
		}
	});
});
</script>
