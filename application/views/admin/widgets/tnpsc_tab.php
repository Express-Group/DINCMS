...
<ul class="resp-tabs-list hor_1 ">';
			...
			$widget_main_section_id = "";
				...
				$add_attr = ($l>1)?'id="tnpsc'.$tab_id.'" data-url="'.$get_section['URLSectionStructure'].'" ': '';
...
if(count($widget_contents)>0)
{
   ...
   }elseif($view_mode=="adminview")
  {
        ...
        $show_simple_tab .='<div class="margin-bottom-10"></div>';
  		...
  		$show_simple_tab .='</ul> </div></div></div></div></div>'; 	

}
else
   {
	 $tab_id  = 	($content['RenderingMode'] == "manual") ? $get_section['WidgetInstanceMainSection_id'] : $get_section['Section_ID'];
			$show_simple_tab .='<div id="tnpsc_'.$tab_id.'_'.$widget_instance_id.'">
...
$('#parentHorizontalTab<?php echo $widget_instance_id; ?>').easyResponsiveTabs({activate: function(event, tab) {
	...
	$('#parentHorizontalTab_<?php echo $widget_instance_id; ?> .resp-tab-content').not('#tnpsc_'+tnpsc_id+'_<?php echo $widget_instance_id; ?>').css('display','none');
	if($(this).attr('id'))
	{
		$.ajax({
			url:'<?php echo base_url(); ?>user/commonwidget/tnpsc_content',
			method:'post',
			data:{section_id:tnpsc_id, widgetinstanceid:'<?php echo $widget_instance_id; ?>',mode: '<?php echo $content['mode'];?>', 'rendermode' : '<?php echo $content['RenderingMode'];?>', is_home : '<?php echo $is_home;?>', max_article : '<?php echo $content['show_max_article'];?>', summary_option: '<?php echo $is_summary_required;?>',param : '<?php echo $content['close_param'];?>'},
			beforeSend	: function() {				
								},
