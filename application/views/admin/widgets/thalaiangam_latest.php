<style>
.box-one-thalaiangam-latest{height:290px !important;}
.arrow-grey-custom{text-align:right;}
.arrow-grey-custom a{font-weight:bold;}
.thalaiangam_1_body{background-color: #FFE9E8;border: 1px solid rgba(228,114,151,.21);}
.thalaiangam_2_body{background-color: #e6f3cc; border: 1px solid rgba(118,140,72,.18);}
.thalaiangam_1_title{background-color: #E47297;}
.thalaiangam_2_title{background-color: #E47297;}
</style>
<?php 

$widget_bg_color = $content['widget_bg_color'];
$widget_type=str_replace(array("style='background-color:",";'"),'',$content['widget_bg_color']);
...
if($widget_type==1){
	$tiltle_class='thalaiangam_1_title';
	$body_class='thalaiangam_1_body';
}else if($widget_type==1){
	$tiltle_class='thalaiangam_2_title';
	$body_class='thalaiangam_2_body';
}else{
	$tiltle_class='';
	$body_class='';
}
...
$show_simple_tab 	.='<div class="row">
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
          ...
          <div class="sports-padd">';
																		...
																		foreach ($widget_instance_contents1 as $key1 => $value1) {
																			if($value['content_id']==$value1['content_id']){
																				$widget_contents[] = array_merge($value, $value1);
																												...
																												$widget_section_url = $domain_name .$get_section['URLSectionStructure'];														//getting content block ends here
														//Widget code block - code required for simple tab structure creation. Do not delete
														//Widget code block Starts here
														if($j==0){
														$add_class='box-botton font-size-11 '.$tiltle_class;
														$add_sub_class='box-one box-one-thalaiangam-latest '.$body_class.'';
														}
														elseif($j==1){
														$add_class='box-botton box-botton1 font-size-11';
														$add_sub_class='box-one box-one1 pair-one cartoon-sec';	
														}
														
														$show_simple_tab .='<div class="col-lg-12 col-md-12 col-sm-6 col-xs-12  cart">';
				...
				elseif($j==1)
				{ //echo $original_image_path;
				
				if($manual_image_path!='')
				{
						$show_image = image_url. imagelibrary_image_path . $manual_image_path;
						
						//echo'manual';
				}else if ($original_image_path!='' && get_image_source($original_image_path, 1))
					{
					$imagedetails = get_image_source($original_image_path, 2);
					$imagewidth = $imagedetails[0];
					$imageheight = $imagedetails[1];	
						...
						if ($imageheight > $imagewidth)
						{
						$Image120X180 	= $original_image_path;
						}
						else
						{				
						$Image120X180 	= str_replace("original","w120X180", $original_image_path);
						}
					$show_image = image_url. imagelibrary_image_path . $Image120X180;
					$manual_image_path= $Image120X180;
					//echo image_url . imagelibrary_image_path .$manual_image_path;
					}
				
					if(!get_image_source($manual_image_path, 1))
					{
						//echo $show_image;
				...
				//  summary block endss here
													...
													if($i == 1)
													{
													 ...
													 $show_simple_tab .='<a href="'.$live_article_url.'" class="article_click" > 
															...
															$show_simple_tab.='<p class="summary font-size-11">'.$summary.'</p>';
															
															}elseif($j==1){
															 $show_simple_tab .='<figcaption><p>'.$display_title.'</p>';
															...
															$show_simple_tab .='<div class="arrow-grey-custom"><a  href="'.$widget_section_url.'">மேலும் படிக்க  <i class="fa fa-angle-double-right" aria-hidden="true"></i> </a></div>';
															//$show_simple_tab .='<div class="arrow-grey medai"><a href="'.$widget_section_url.'" onclick="call_url('.$section_landing.')"><div class="arrow-span"> </div><div class="arrow-rightnew"></div></a></div>';
