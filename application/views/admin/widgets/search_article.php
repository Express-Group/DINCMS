...
$manual_instance        = $this->input->get('widget2_instance');
...
$manual_instance                 = ($datafrom=="live" || $datafrom=="")? $this->input->get('widget2_instance') : "";
$search_term_ids                 = "";
$searchby_section                = "";
...
$instance1             = ($widget1_instance!='') ?  "&widget1_max=".$widget1_max."&widget1_mode=".$widget1_mode."&widget1_instance=".$widget1_instance : '';
$manual_instance       = $widget2_instance."&widget2_max=".$widget2_max."&widget2_mode=".$widget2_mode.$instance1;
}
}else{
$datafrom                = date('Y');  
}
...
$instance        = ($manual_instance!='' && $manual_instance!="archive") ? "&widget2_instance=".$manual_instance : ""; 
$sid_url         = ($sid !='')? "&sid=".$sid.$instance: "";
$load_more_sid_url = ($sid !='')? "&sid=".$sid : "";
if($fromdate!='' || $todate!=''){
...
<li class="even">
		...
		$datafrom = ($datafrom=="live" || $datafrom=="")? "" : $loaddata_from;
		if($type_search!=3){
        ...
        <li>முடிவினைக் காட்டு <span id="ordering" class="active"><a href="<?php echo $query_url.$content_type."&stype=".$type_search."&datafrom=".$datafrom."&order_dir=".$order.$order_by.$sid_url;?>"><?php if(isset($order)){ if($order =="Asc"){?> இறங்குவரிசை  <?php }else{?> ஏறுவரிசை <?php }}?></a></span> ஒழுங்கு</li>
       <?php if($sid ==''){ ?>
        <li>
     ...
     $custom_title     = preg_replace('/<p[^>]*>(.*)<\/p[^>]*>/i', '$1',$custom_title);
		...
		{
			$datafrom = ($datafrom=="live" || $datafrom=="")? date('Y') : $loaddata_from-1;
			$acrchive_url = $query_url.$content_type."&stype=".$type_search."&datafrom=".$datafrom.$load_more_sid_url;
...
?>
 <ul class="ascending" id="table_sorter"><li style="float:left;">தேடல் முடிவுகள் உள்ள : <span class="active"><?php echo $search_term;?></span></li></ul>
        ...
        <?php if($loaddata_from!="table_not_exist"){ ?>
        <?php if($search_term!=''){ ?>
    ...
    var datePicker2 = $("#example2").datepicker(dpOptions).
		...
		$("#searchbtn").click(function() 
		{
		   ...
		   var search_type = $('input[name=search_type]').val();
			if(search_term.trim() =='')
			{
				$('#emptySearchExpressionError').text("தேடும் சொல்லை சொற்களை உள்ளிடவும்").show();
			...
			}else
			{
				...
				{
				$('#emptySearchExpressionError').text("200 க்கும் மேற்பட்ட எழுத்துக்கள் உள்ளிட முடியாது!").show();
				$('.validate').addClass('error');
				return false;
				}
				if(search_type ==''){
