...
$show_simple_tab 	    = 	"";
$looptype = 4;
$className = "col-lg-3 col-md-3 col-sm-6 col-xs-12 SundaySecondSplit";
...
$show_simple_tab.='<h5 class="din-title"><a>உலகம்</a></h5>';
if(strtolower(strip_tags($widget_custom_title))!='none'){
	//$show_simple_tab.='<h5 class="din-title">உலகம்</h5>';
}	
...
{
	$splitTitle = explode(',',$widget_custom_title);
	foreach($widget_contents as $get_content) // For Get Content Start Here 
	{
		...
		$spantitle ='';
		if(isset($splitTitle[$i-1]) && $splitTitle[$i-1]!=''):
			$spantitle = '<span style="color:#c70000;">'.trim($splitTitle[$i-1]).'</span>';
		endif;
		$display_title = '<a  href="'.$live_article_url.'" class="article_click" >'.$spantitle.$display_title.'</a>';
		...
		if($count <= $looptype)
		{
			...
			$show_simple_tab.= '<div class="WidthFloat_L margin-bottom-15">'; 
			} 
			
		$show_simple_tab.= '<div class="'.$className.'">';
		$show_simple_tab.= '<div class="sa-2">';
		if(isset($splitTitle[$i-1]) && $splitTitle[$i-1]!=''):
			//$show_simple_tab.= '<div class="pronews-section"><span>'.trim($splitTitle[$i-1]).'</span></div>';
		endif;
		...
		$show_simple_tab .='<p class="sa-1">'.$display_title.'</p></a>';
