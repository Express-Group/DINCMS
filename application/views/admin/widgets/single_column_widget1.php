...
if(count($widget_instance_contents)>0){			
	foreach($widget_instance_contents as $get_content){
		...
		if($i==1){
			...
			$show_simple_tab 	   .=	'<div class="widget-4-block" '.$widget_bg_color.'>';}
		...
		if($i==1){	
			$show_simple_tab.= '<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 widget-block-item single-col-main m1">';
		}else{
			$show_simple_tab.= '<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 widget-block-item single-col-main">';
		}
		if($i==1){
			$show_simple_tab.= '<a  href="'.$live_article_url.'" class="article_click"  ><img style="margin-top:10px;" src="'.$dummy_image.'" data-src="'.$show_image.'" title = "'.$imagetitle.'" alt = "'.$imagealt.'">';
		}
		if($content_type == '3' && $i==1){
