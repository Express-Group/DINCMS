	...
	<div class="col-md-12">
		...
		<?php
			...
			foreach($Result as $fetched):
					...
					$u=image_url;
					$fetched->article_page_image_path=($fetched->article_page_image_path=='')?'logo/dinamani_logo_600X390.jpg':$fetched->article_page_image_path;
					$low='logo/dinamani_logo_600X390.jpg';
					
					print '<tr>';
					print '<td><figure class="result-section-figure"><img src="'.$u.imagelibrary_image_path.$low.'" data-src="'.$u.imagelibrary_image_path.$fetched->article_page_image_path.'" class="lazy-loaded"></figure></td>';
...
<div class="adv_search_result">
	...
	if($tags!=''){
		...
		$cookie_tag=@$_COOKIE['tag_'.$tag_type.str_replace(' ','_',substr(urlencode($tags), 0 ,10))];
		...
		if($cookie_tag==base64_encode($tag_query)){
			...
			setcookie('tag_'.$tag_type.str_replace(' ','_',$tags), base64_encode($tag_query),time() + (60 * 15));
	...
	if(count($Result) > 0):
		if($term=='') { $title= urldecode($this->uri->segment(2)); $title_type='Tag';} else { $title= $term ; $title_type='Search';}
	...
	foreach($Result as $fetched):
			...
			print '<tr>';
			print '<td><figure class="result-section-figure"><img src="'.image_url.imagelibrary_image_path.$fetched->article_page_image_path.'"></figure></td>';
			print '<td><div class="search-row_type"><h4><a class="article_click" href="'.BASEURL.$fetched->url.'">'.strip_tags($fetched->title).'</a></h4><p>'.strip_tags($fetched->summary_html).'</p><date>published on : '.$publisheddate.'</date></div></td>';
			print '</tr>';
	endforeach;
	print '</table>';
