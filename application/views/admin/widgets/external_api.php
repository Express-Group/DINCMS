...
/*get site url */
	...
	elseif($render_mode=='dn'):
		...
		$api_url='https://www.dinamani.com/new-sitemap.xml';
...
$part .='<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 padding-0"><a target="_BLANK" href="'.$external_site.'"><div class="tamilnews_heading external_heading_'.$class.'">'.$widget_custom_title.'</div></a></div>';

$part_new .='<div class="external_api_nie nie_api_'.$class1.'">';
$part_new .='<h4 class="external_api_header_nie"><a target="_BLANK" href="'.$external_site.'">'.$title_new.'</a></h4>';
$part .='</div>';
...
?>
<style>
.external_heading_<?php print $class ?>:before {
 color: red;
 content: "<?php print $title[0] ?> ";
}
</style>
<?php
if($status==405):
	...
	?>
		<script>
			...
			function load_external_api_nie(widget_instance_id,widget_instance_id1,widget_title,external_url,template_type,widget_title_new){
				...
				if(render_mode=='dn'){
					var dummy_image='https://images.dinamani.com/uploads/user/imagelibrary/logo/dinamani_logo_150X150.jpg';
					var dummy_image1='https://images.dinamani.com/uploads/user/imagelibrary/logo/dinamani_logo_100X65.jpg';
				}else if(render_mode=='sm'){
			var dummy_image='https://images.samakalikamalayalam.com/uploads/user/imagelibrary/logo/nie_logo_150X150.jpg';
		var dummy_image1='https://images.samakalikamalayalam.com/uploads/user/imagelibrary/logo/nie_logo_100X65.jpg';
				...
				$.post(rule_url,{ url: "<?php print $api_url ?>" }, function(data){
					 ...
					 var template='';
					var template_new='';
					template +='<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 padding-0"><a target="_BLANK" href="'+external_url+'"<div class="tamilnews_heading external_heading_'+widget_instance_id+'">'+widget_title+'</div></a></div>';
					template_new +='<h4 class="external_api_header_nie"><a target="_BLANK" href="'+external_url+'">'+widget_title_new+'</a></h4>';
					...
					$(data).find(xml).each(function(index) {
						...
						image=image.replace('original','w150X150');
						if(render_mode=='kp'){
