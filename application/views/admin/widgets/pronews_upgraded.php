...
$show_simple_tab 	    = 	"";
$looptype = 4;
$className = "col-lg-3 col-md-3 col-sm-3 col-xs-12 SundaySecondSplit";
if($is_home=='y'){
	//$looptype = 2;
	//$className = "col-lg-6 col-md-6 col-sm-6 col-xs-12 SundaySecondSplit";
}
...
if(count($widget_contents)>0)
{
	...
	foreach($widget_contents as $get_content) // For Get Content Start Here 
	{
		...
		if($count <= $looptype)
		{
			...
			$show_simple_tab.= '<div class="WidthFloat_L pronews-list-wrapper margin-bottom-15">'; 
			} 
		...
		$show_simple_tab.= '<div class="pronews-list"><a  href="'.$live_article_url.'" class="article_click"  >
		<img src="'.$dummy_image.'" data-src="'.$show_image.'" title = "'.$imagetitle.'" alt = "'.$imagealt.'">';
		$show_simple_tab .='<p>'.$display_title.'</p></a>';
