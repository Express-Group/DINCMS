...
$show_simple_tab 	   .=	'<div class="col-md-12 col-lg-12 col-sm-12 col-xs-12">';
...
if(count($widget_instance_contents)>0){
	$show_simple_tab .=	'<div class="widget-3-block margin-bottom-20" '.$widget_bg_color.'>';
	$dummy_image1	= image_url. imagelibrary_image_path.'logo/dinamani_logo_600X390.jpg';
	foreach($widget_instance_contents as $get_content){
		...
		if($original_image_path !='' &&  get_image_source($original_image_path, 1)){
			...
			if ($Image600X300 != '' && get_image_source($Image600X300, 1)){
				...
				$show_image1 = image_url. imagelibrary_image_path . str_replace("original","w600X390", $original_image_path);
				$is_image = true;
			}else{
				$show_image	= image_url. imagelibrary_image_path.'logo/dinamani_logo_600X300.jpg';
				$show_image1	= image_url. imagelibrary_image_path.'logo/dinamani_logo_600X390.jpg';
			}
		...
		if($i==1){
			...
			$show_simple_tab.= '<a  href="'.$live_article_url.'" class="article_click"  ><img src="'.$dummy_image.'" data-src="'.str_replace('300' ,'390' , $show_image).'" title = "'.$imagetitle.'" alt = "'.$imagealt.'">';
			if($content_type == '3'){
				$show_simple_tab.='<img class="GalleryIcon" style="top: 8px;"  src="'.$gallery_icon.'">';
			}else if($content_type == '4'){
				$show_simple_tab.='<img class="GalleryIcon" style="top: 8px;"  src="'.$play_video_image.'">';
			}
			$show_simple_tab .='</a><h4 class="topic-head">'.$display_title.'</h4>';
		...
		if($i > 1){
			...
			if($i==2){ $show_simple_tab.= '<div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">'; }
			$show_simple_tab.= '<div class="WidthFloat_L">';
			$show_simple_tab.= '<a  href="'.$live_article_url.'" class="article_click"  ><img src="'.$dummy_image.'" data-src="'.$show_image.'" title = "'.$imagetitle.'" alt = "'.$imagealt.'">';
			if($content_type == '3'){
				$show_simple_tab.='<img class="GalleryIcon" style="top: 8px;"  src="'.$gallery_icon.'">';
			}else if($content_type == '4'){
				$show_simple_tab.='<img class="GalleryIcon" style="top: 8px;"  src="'.$play_video_image.'">';
