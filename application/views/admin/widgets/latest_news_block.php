...
// content list iteration block starts here
   ...
   $count=count($widget_contents);
	if(count($widget_contents)>0)
	{
		...
		$responseData = array_chunk($widget_contents ,round(count($widget_contents) /$widgettype));
		$show_simple_tab .='<div class="latest-news-block-inner">';
		for($m=0;$m<count($responseData);$m++){
			if(count($responseData[$m]) > 0){
				$show_simple_tab .='<div class="col-md-'.$colvalue.' col-lg-'.$colvalue.' col-sm-'.$colvalue.' col-xs-12">';
				$show_simple_tab .='<ul>';
			}
