	...
	$Image150X150 	= str_replace(".","_150_150.", $ImageName);
	  ...
	  if (file_exists(source_base_path . $SourceURL . $ImageName))
	  {
		  if (copy(source_base_path . $SourceURL . $ImageName, destination_base_path . $DestinationURL.$FolderMapping. $NewImageName)){
			unlink(source_base_path . $SourceURL .$ImageName);
			chmod(destination_base_path . $DestinationURL.$FolderMapping. $NewImageName, 0777);
			if($CI->input->post('watermark')=='on'){
				$config['source_image'] = destination_base_path.$DestinationURL.$FolderMapping. $NewImageName;
                $config['wm_overlay_path'] = source_base_path.'images/watermark/450x140.png';
	   ...
	   if (file_exists(source_base_path . $SourceURL . $Image600X390))
	  {
		  if (copy(source_base_path . $SourceURL . $Image600X390, destination_base_path . $DestinationURL.$ImageFolder600X390 . $NewImageName)) {
			...
			if($CI->input->post('watermark')=='on'){
				//$CI->load->library('image_lib');
				$config['source_image'] = destination_base_path.$DestinationURL.$ImageFolder600X390 . $NewImageName;
                $config['wm_overlay_path'] = source_base_path.'images/watermark/450x140.png';
                $config['wm_type'] = 'overlay';
                $config['wm_opacity'] = '100';
                $config['wm_vrt_alignment'] = 'middle';
	 ...
	 // echo source_base_path . $SourceURL . $Image600X390. destination_base_path . $DestinationURL.$ImageFolder600X390 . $NewImageName;
	   ...
	   if (file_exists(source_base_path . $SourceURL . $Image600X300))
	  {
		  ...
		  if (copy(source_base_path . $SourceURL . $Image600X300, destination_base_path . $DestinationURL.$ImageFolder600X300. $NewImageName))
			...
			if($CI->input->post('watermark')=='on'){
				//$CI->load->library('image_lib');
				$config['source_image'] = destination_base_path.$DestinationURL.$ImageFolder600X300 . $NewImageName;
                $config['wm_overlay_path'] = source_base_path.'images/watermark/450x140.png';
                $config['wm_type'] = 'overlay';
                $config['wm_opacity'] = '100';
                $config['wm_vrt_alignment'] = 'middle';
	   ...
	   if (file_exists(source_base_path . $SourceURL . $Image100X65))
	  {
		  if (copy(source_base_path . $SourceURL . $Image100X65, destination_base_path . $DestinationURL.$ImageFolder100X65. $NewImageName))
			...
			if($CI->input->post('watermark')=='on'){
				//$CI->load->library('image_lib');
				$config['source_image'] = destination_base_path.$DestinationURL.$ImageFolder100X65. $NewImageName;
                $config['wm_overlay_path'] = source_base_path.'images/watermark/95x30.png';
                $config['wm_type'] = 'overlay';
                $config['wm_opacity'] = '100';
                $config['wm_vrt_alignment'] = 'middle';
                $config['wm_hor_alignment'] = 'center';
	   ...
	   if (file_exists(source_base_path . $SourceURL . $Image150X150))
	  {
		  if (copy(source_base_path . $SourceURL . $Image150X150, destination_base_path . $DestinationURL.$ImageFolder150X150. $NewImageName))
			...
			if($CI->input->post('watermark')=='on'){
				//$CI->load->library('image_lib');
				$config['source_image'] = destination_base_path.$DestinationURL.$ImageFolder150X150. $NewImageName;
                $config['wm_overlay_path'] = source_base_path.'images/watermark/125x39.png';
                $config['wm_type'] = 'overlay';
                $config['wm_opacity'] = '100';
                $config['wm_vrt_alignment'] = 'middle';
                $config['wm_hor_alignment'] = 'center';
...
#START: This only for Dinamani

function get_sectionDetails_by_name($section_name)
{
	if($section_name != '') {
	$CI = & get_instance();	
	
	$section_details = $CI->db->query("CALL get_sectionid_by_name('" . $section_name . "')");
	$row_array = $section_details->row_array();
	
	return @$row_array['Section_id'];
	} else {
	return '';
	}
...
function numerology_list($number_id,$monthName){
	$CI = & get_instance();
	
	$general_qry =  $CI->db->query('SELECT prediction_details FROM numerology_general_predictions WHERE section_id='.$number_id);
	$result['general_details'] = '';
	if($general_qry->num_rows() > 0){
		$general_result = $general_qry->row_array();
		$result['general_details'] = ($general_result['prediction_details'] != '') ? $general_result['prediction_details'] : '';
	...
	$daily_qry =  $CI->db->query('SELECT prediction_details FROM numerology_daily_predictions WHERE prediction_date IN (SELECT max(prediction_date) FROM numerology_daily_predictions WHERE section_id='.$number_id.')');
	$result['daily_details'] = '';
	if($daily_qry->num_rows() > 0){
		$daily_result = $daily_qry->row_array();
		$result['daily_details'] = ($daily_result['prediction_details'] != '') ? $daily_result['prediction_details'] : '';
	...
	$weekly_qry =  $CI->db->query("SELECT prediction_details FROM numerology_weekly_predictions WHERE section_id='".$number_id."' AND prediction_date <= '".$date."' AND prediction_end_date >= '".$date."' ");
	$result['weekly_details'] = '';
	if($weekly_qry->num_rows() > 0){
		$weekly_result = $weekly_qry->row_array();
		$result['weekly_details'] = ($weekly_result['prediction_details'] != '') ? $weekly_result['prediction_details'] : '';
		...
		$weekly_qry =  $CI->db->query("SELECT prediction_details FROM numerology_weekly_predictions WHERE prediction_date IN (SELECT max(prediction_date) FROM numerology_weekly_predictions WHERE section_id='".$number_id."') AND prediction_date <= '".$date."' ");
		$result['weekly_details'] = '';
		if($weekly_qry->num_rows() > 0){
			$weekly_result = $weekly_qry->row_array();
			$result['weekly_details'] = ($weekly_result['prediction_details'] != '') ? $weekly_result['prediction_details'] : '';
	...
	/*$monthly_qry =  $CI->db->query("SELECT prediction_details FROM numerology_monthly_predictions WHERE section_id='".$number_id."' AND prediction_date <= '".$date."' AND prediction_end_date >= '".$date."' ");*/
	
	$monthly_qry =  $CI->db->query("SELECT prediction_details FROM numerology_monthly_predictions WHERE section_id='".$number_id."' AND prediction_date <= '".$date."' AND prediction_month = '".$monthName."' ");
	$result['monthly_details'] = '';
	if($monthly_qry->num_rows() > 0){
		$monthly_result = $monthly_qry->row_array();
		$result['monthly_details'] = ($monthly_result['prediction_details'] != '') ? $monthly_result['prediction_details'] : '';
		...
		/*$monthly_qry =  $CI->db->query("SELECT prediction_details FROM numerology_monthly_predictions WHERE prediction_date IN (SELECT max(prediction_date) FROM numerology_monthly_predictions WHERE section_id='".$number_id."') AND prediction_date <= '".$date."' ");*/
		$monthly_qry =  $CI->db->query("SELECT prediction_details FROM numerology_monthly_predictions WHERE section_id='".$number_id."' AND prediction_date IN (SELECT max(prediction_date) FROM numerology_monthly_predictions WHERE section_id='".$number_id."') AND  prediction_date <= '".$date."' ");
		if($monthly_qry->num_rows() > 0){
			$monthly_result = $monthly_qry->row_array();
			$result['monthly_details'] = ($monthly_result['prediction_details'] != '') ? $monthly_result['prediction_details'] : '';
		}
	...
	$result['tamil_monthly_details'] = '';
	$tamil_monthly =  $CI->db->query("SELECT prediction_details FROM numerology_monthly_predictions WHERE section_id='".$number_id."'  AND  '".$date."'  BETWEEN `prediction_date` AND `prediction_end_date` ");
	if($tamil_monthly->num_rows() > 0){
	$tamil_monthly_res = $tamil_monthly->row_array();
	//echo $CI->db->last_query();exit;
	$result['tamil_monthly_details'] = ($tamil_monthly_res['prediction_details'] != '') ? $tamil_monthly_res['prediction_details'] : '';
	...
	$yearly_qry =  $CI->db->query("SELECT prediction_details FROM numerology_yearly_predictions WHERE section_id='".$number_id."' AND YEAR(prediction_date) = '".date('Y')."'  ");
	$result['yearly_details'] = '';
	if($yearly_qry->num_rows() > 0){
		$yearly_result = $yearly_qry->row_array();
		$result['yearly_details'] = ($yearly_result['prediction_details'] != '') ? $yearly_result['prediction_details'] : '';
...
function tamil_month($current_month)
	{
		switch($current_month){
			case 'January':
				$month_name_tamil = 'ஜனவரி';
				break;
			case 'February':
				$month_name_tamil = 'பிப்ரவரி';
				break;
			case 'March':
				$month_name_tamil = 'மார்ச்';
				break;
			case 'April':
				$month_name_tamil = 'ஏப்ரல்';
				break;
			case 'May':
				$month_name_tamil = 'மே';
				break;
			case 'June':
				$month_name_tamil = 'ஜூன்';
				break;
			case 'July':
				$month_name_tamil = 'ஜூலை';
				break;
			case 'August':
				$month_name_tamil = 'ஆகஸ்ட்';
				break;
			case 'September':
				$month_name_tamil = 'செப்டம்பர்';
				break;
			case 'October':
				$month_name_tamil = 'அக்டோபர்';
				break;
			case 'November':
				$month_name_tamil = 'நவம்பர்';
				break;
			default:
				$month_name_tamil = 'டிசம்பர்';	
		}
		return $month_name_tamil;
	
	}
	
	function getSmallLink($longurl){  
    $apiURL = 'https://www.googleapis.com/urlshortener/v1/url?key=AIzaSyBScw4oGoOpLpCVUj2tzvxeRAY8P6yxLKs'; //AIzaSyBoXB8LqB7x_nFA4VizAzFpPqNp0FRk9pw //AIzaSyCeeQHget_bZHy3P_qI2wnQWhGItnbudgU
...
function get_image_source($image_path, $condition_check){ //$condition_check=> 1 - file_exists, 2 - height,wieght
	//on file uploading to live change this file_exists => getimagesize and destination_base_path => image_url
	if($condition_check==1){
		return file_exists(destination_base_path . imagelibrary_image_path.$image_path); //getimagesize - image_url
	}else if($condition_check==2){
		return getimagesize(destination_base_path . imagelibrary_image_path.$image_path); //image_url
	}
}

function time_elapsed_string($datetime, $full = false ,$type=1) {
    $now = new DateTime;
...
function amp_validation($url='',$articleId=0 , $archive=''){ 
	$timestamp = strtotime("now");
	if($archive!=''){
		$url= AMPURL.$articleId.'/article/'.$timestamp.'/amp_validation.cloud?archive='.$archive;	
	}else{
		$url= AMPURL.$articleId.'/article/'.$timestamp.'/amp_validation.cloud';	
	}
	$ampurl = 'https://amp.cloudflare.com/q/'.$url;
	$ch = curl_init();
	...
	$valid = ($result['valid']==1) ? 1 : 0;
	$CI = & get_instance();
	$live_db = ($archive!='') ? $CI->load->database('archive_db' , TRUE) : $CI->load->database('live_db' , TRUE);
	if($archive!=''){
		if($live_db->table_exists('article_'.$archive)){
			$live_db->query("UPDATE article_".$archive." SET amp_status='".$valid."' WHERE content_id='".$articleId."'");
		}
	}else{
		$CI->db->query("UPDATE articlemaster SET amp_status='".$valid."' WHERE content_id='".$articleId."'");
		$live_db->query("UPDATE article SET amp_status='".$valid."' WHERE content_id='".$articleId."'");
	}
} 
	
	#END: This only for Dinamani
