	...
	//	error_log('CALL get_astro_DailyPrediction(" ORDER BY '.$order_field.' '.$order.' LIMIT '.$start.', '.$length.'","'.$from_date.'","'.$to_date.'","'.$rasi_id.'")');
		...
		foreach($dailyPrediction_values as $daily_prediction) {
			...
			if(defined("USERACCESS_EDIT".$Menu_id) && constant("USERACCESS_EDIT".$Menu_id) == '1'){
				$set_rights .= '<div class="buttonHolder"><a class="button heart tooltip-3"  href="'.base_url().'dmcpan/astro_daily_prediction/update_daily_predictions/'.urlencode(base64_encode($daily_prediction['prediction_id'])).'" data-toggle="tooltip" title="Edit"> <i class="fa fa-pencil" ></i> </a>';
			} else { $set_rights.="";			}
	...
	public function insert_update($user_id)
	{
		...
		if($hidden_id == "")
		{
			...
			if($this->db->trans_status() == FALSE)
			{
				...
				redirect('dmcpan/astro_daily_prediction');
			}
			else
			{
