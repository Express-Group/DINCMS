...
class Widget_model extends CI_Model
{
  ...
  public function __construct() 
	...
	public function trending_tagslist($fromDate , $endDate ,$type=1){
		if($type==1){
			$CacheID = "SELECT article.url , article.tags , tag.tag_name FROM article AS article INNER JOIN seo_tags AS tag ON article.content_id=tag.content_id WHERE article.content_id IN(SELECT content_id FROM content_hit_history WHERE content_type=1 AND published_on  BETWEEN '".$fromDate."' AND '".$endDate."' ORDER BY hits DESC) AND article.status='P' AND tag.tag_name!='' LIMIT 5";
		}else{
			$CacheID = "SELECT article.url , article.tags , tag.tag_name FROM article AS article INNER JOIN seo_tags AS tag ON article.content_id=tag.content_id WHERE article.status='P' AND article.tags!='' AND article.publish_start_date < NOW() ORDER BY article.publish_start_date DESC LIMIT 5";
		}
	...
	public function get_all_available_articles_auto_other_stories($max_article, $section_id, $view_mode, $lead_story_maximum_Articles , $subsec_leadstory_remdering_mode, $leadstory_contentID)
	 ...
	 public function get_details_with_bodycontent($content_id,$content_type,$is_home, $view_mode)
	 {
	  $class_object = new Widget_config_class;
	  return $class_object->get_details_with_bodycontent($content_id,$content_type,$is_home, $view_mode);
	 }
	 ...
	 public function get_liveContents_by_sectionId_count_array_old($max_article, $section_id, $view_mode, $lead_story_maximum_Articles , $subsec_leadstory_remdering_mode, $leadstory_contentID, $contentType_id,$is_home)
	 {
	  $class_object = new Widget_config_class;
	  return $class_object->get_liveContents_by_sectionId_count_array_old($max_article, $section_id, $view_mode, $lead_story_maximum_Articles , $subsec_leadstory_remdering_mode, $leadstory_contentID, $contentType_id,$is_home);
	 }
	...
	public function get_widget_Poll_data($view_mode)
	{
		$class_object = new Widget_config_class;
		return $class_object->get_widget_Poll_data($view_mode);
	}

	public function get_widget_breakingNews_contents($view_mode)
	{
		$class_object = new Widget_config_class;
		return $class_object->get_widget_breakingNews_contents($view_mode);
	}
	...
	public function get_widgetInstanceArticles_rendering($widget_instance, $mainSectionConfig_Id, $view_mode, $max_article=0)
	{
		$class_object = new Widget_config_class;
	...
	public function get_content_by_id($content_id, $content_typeid, $url){
	    $class_object =  new Widget_content_class;
		return $class_object->get_content_by_id($content_id, $content_typeid, $url);
	}
 ...
 public function get_content_type($content_id)
	...
	public function widget_archieve_content_by_section($sectionid, $table)
	{
	 $class_object = new Widget_content_class_archieve;
		return $class_object->widget_archieve_content_by_section($sectionid, $table);
	}
...
class Widget_config_class extends Widget_model
{
	...
	public function get_widget_breakingNews_contents($view_mode){
		if($view_mode == 'live')
		$breaking_news = $this->live_db->query("SELECT breakingnews_id, Title, Displayorder, Content_ID, Createdon, Modifiedon FROM breakingnewsmaster");
		else
	...
	public function GetLiveNowContent($type,$article_id,$image_url){
		...
		if($type==1){
			...
			foreach($Result as $Data):
				...
				$Template .='<div class="livenow-socialicons"><a target="_blank" href="https://www.facebook.com/sharer/sharer.php?u='.BASEURL.'&title='.$Data->title.'&picture='.$image_url.'"><i class="fa fa-facebook" aria-hidden="true"></i></a><a target="_blank" href="https://twitter.com/intent/tweet?text='.$Data->title.' via @NewIndianXpress" data-size="large" ><i class="fa fa-twitter" aria-hidden="true"></i></a></div>';
...
public function get_sub_sec_lead_stories_data($Pagesection_id, $Page_type, $WidgetDisplayOrder, $widgetId, $section_id, $version_id,  $view_mode)
 {
	 if($view_mode == "live")
	 {
		...
		$result = $subsection_leadstory->row_array();
		$this->memcached_library->add($CacheID,$result);
			} else {
				 $result = $this->memcached_library->get($CacheID);
			}
...
public function get_widget_byname($widgetname, $view_mode)
 ...
 {
	$CacheID="CALL get_gallery_images_by_id('".$content_id."')";
	 if(!$this->memcached_library->get($CacheID) && $this->memcached_library->get($CacheID) == '') {
...
public function get_sectionid_by_article($contentid, $view_mode)
	{
		if($view_mode == "live")
		{
			$CacheID="CALL get_sectionid_by_article('".$contentid."')";
			if(!$this->memcached_library->get($CacheID) && $this->memcached_library->get($CacheID) == '') {	 
			$sectionid = $this->live_db->query("CALL get_sectionid_by_article('".$contentid."')");
			$result = $sectionid->result_array();
			 $this->memcached_library->add($CacheID,$result);
				} else {
					 $result = $this->memcached_library->get($CacheID);
				}
		...
		{
			$result = $this->db->query("CALL get_sectionid_by_article('".$contentid."')")->result_array();
...
{ 
    ...
    $length   = ($subsec_leadstory_remdering_mode == "manual")? '' : $max_article;
    $Section   = $section_id;
    $Status   = "P";
    $content_type  = $contentType_id;
	
	/*if($subsec_leadstory_remdering_mode == "manual" || $subsec_leadstory_remdering_mode == "auto" || $subsec_leadstory_remdering_mode= "search_mode")
	...
	if($view_mode == "live")
	{
		$CacheID='CALL other_stories_contents ("'.$leadstory_contentID.'","'.$Section.'","'.$start_limt.'", "'.$length.'","'.$is_home.'", "'.$contentType_id.'", "'.$subsec_leadstory_remdering_mode.'")';
		if(!$this->memcached_library->get($CacheID) && $this->memcached_library->get($CacheID) == '') {	 
		$article_manager  =  $this->live_db->query('CALL other_stories_contents ("'.$leadstory_contentID.'","'.$Section.'","'.$start_limt.'", "'.$length.'","'.$is_home.'", "'.$contentType_id.'", "'.$subsec_leadstory_remdering_mode.'")');
 ...
 public function get_contentdetails_from_database_per_page($content_id,$content_type,$is_home , $view_mode, $start,$limit)
 {
	...
	/* if($view_mode == "live"){
	  $CacheID='CALL  get_contentdetails_from_live_per_page ("'.$content_id.'","'.$content_type.'","'.$is_home.'" , "'.$start.'", "'.$limit.'")';
	if(!$this->memcached_library->get($CacheID) && $this->memcached_library->get($CacheID) == '') {	
 ...
 {
		$CacheID='CALL  get_all_available_articles_auto_live ("'.$content_id.'","'.$content_type.'","'.$is_home.'")';
		if(!$this->memcached_library->get($CacheID) && $this->memcached_library->get($CacheID) == '') {	
...
public function get_widget_mainsection_config_rendering($instance_mainsection_id, $instance_id, $view_mode)
	...
	public function get_widget_Poll_data($view_mode)
	{
		$polls = $this->db->query('CALL get_active_poll("1")')->row_array();
		return $polls;
	}
...
class Search extends Widget_model
{
    ...
    public function get_search_result_data($order_field, $order, $start, $length, $frm_search, $to_search, $search_value, $search_by, $section, $content_type, $type_search, $datafrom)
	{
		...
		if($order_field!=''){
		if($order_field == "last_updated_on" || $order_field == "Title" || $order_field == "publish_start_date")
		{
...
*/
	...
	public function get_search_result_data_recursive_em($order_cond, $order, $start, $length, $frm_search, $to_search, $search_value, $search_by, $section, $content_type, $type_search, $table)
	{
		$archive_db    = $this->load->database('archive_db', TRUE);
		$table_exist  = $archive_db->query("show tables like '".$table."'")->num_rows();
		$table_prefix = explode("_", $table);
...
class Widget_content_class extends Widget_config_class {
	...
	public function get_content_by_id($content_id, $content_typeid, $url){
		$CacheID ='CALL widget_article_content_by_id("'.$content_id.'", "'.$content_typeid.'" , "'.urldecode($url).'")';
		if(!$this->memcached_library->get($CacheID) && $this->memcached_library->get($CacheID) == '') {
		$query 	=  $this->live_db->query('CALL widget_article_content_by_id("'.$content_id.'", "'.$content_typeid.'" , "'.urldecode($url).'")');
	...
	public function widget_article_content_by_id($contennt_id, $content_typeid, $url)
	{
		switch($content_typeid){
			case 1:
		$CacheID ='CALL detailed_article_content_by_id("'.$contennt_id.'", "'.urldecode($url).'")';
		if(!$this->memcached_library->get($CacheID) && $this->memcached_library->get($CacheID) == '') {
		$query 	=  $this->live_db->query('CALL detailed_article_content_by_id("'.$contennt_id.'", "'.urldecode($url).'")');
		$content_result = $query->result_array();
				$this->memcached_library->add($CacheID,$content_result);
			}else {
				$content_result = $this->memcached_library->get($CacheID);
			...
			break;
			case 3:
			$CacheID ='CALL detailed_gallery_content_by_id("'.$contennt_id.'", "'.urldecode($url).'")';
		if(!$this->memcached_library->get($CacheID) && $this->memcached_library->get($CacheID) == '') {
		$query 	=  $this->live_db->query('CALL detailed_gallery_content_by_id("'.$contennt_id.'", "'.urldecode($url).'")');
		$content_result = $query->result_array();
				$this->memcached_library->add($CacheID,$content_result);
			}else {
				$content_result = $this->memcached_library->get($CacheID);
			...
			break;
			case 4:
			$CacheID ='CALL detailed_video_content_by_id("'.$contennt_id.'", "'.urldecode($url).'")';
		if(!$this->memcached_library->get($CacheID) && $this->memcached_library->get($CacheID) == '') {
		$query 	=  $this->live_db->query('CALL detailed_video_content_by_id("'.$contennt_id.'", "'.urldecode($url).'")');
		$content_result = $query->result_array();
				$this->memcached_library->add($CacheID,$content_result);
			}else {
				$content_result = $this->memcached_library->get($CacheID);
			...
			break;
			case 5:
			$CacheID ='CALL detailed_audio_content_by_id("'.$contennt_id.'", "'.urldecode($url).'")';
		if(!$this->memcached_library->get($CacheID) && $this->memcached_library->get($CacheID) == '') {
		$query 	=  $this->live_db->query('CALL detailed_audio_content_by_id("'.$contennt_id.'", "'.urldecode($url).'")');
		$content_result = $query->result_array();
				$this->memcached_library->add($CacheID,$content_result);
			}else {
				$content_result = $this->memcached_library->get($CacheID);
		...
		$content_result = $query->result_array();
			...
			/*$CacheID ='CALL widget_article_content_by_id("'.$contennt_id.'", "'.$content_typeid.'" , "'.urldecode($url).'")';
		if(!$this->memcached_library->get($CacheID) && $this->memcached_library->get($CacheID) == '') {
...
class Widget_content_class_archieve extends CI_Model
{
	...
	{
		$archive_db = $this->load->database('archive_db', TRUE);
		$result = $archive_db->query("CALL widget_article_content_by_id ('".$old_article_id."','".$content_type."','".$table."', '".$ecenic."', '".urldecode($current_url)."')");
		//echo $archive_db->last_query();exit;
		$archive_db->close();
