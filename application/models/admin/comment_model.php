...
class datatables  extends Comment_model//datatable search
{
	public function datatable_comment()
	{
		...
		$searchtxt = ($filterby==1)? ((($searchtxt)=='செய்திகள்')? 1: ((($searchtxt)=="புகைப்படங்கள்")? 3 :((($searchtxt)==
"வீடியோக்கள்")? 4:5))): $searchtxt ;
			...
			foreach($block_words as $k=>$v)
		{
			...
			$content_type = ($comments['content_type_id']==1)? "செய்திகள்" : (($comments['content_type_id']==3) ? "புகைப்படங்கள்" : (($comments['content_type_id']==4) ? "வீடியோக்கள்" : "ஆடியோக்கள்" ));
			$subdata[] = $content_type;
			if(strlen($title) > 30)
			{
				$subdata[] ='<p class="tooltip_cursor" href="#" title="'.$title.'">'.mb_substr($title, 0, 20).'...'.'</p>';
			...
			<textarea style="height: 200px;width: 320px !important;" name="txtComments[]" class="CommentPopupText" id="txtComments'.$comments['comments_id'].'" readonly="readonly" title="Updated Comment">'.$comments['UpdatedComment'].'</textarea></div>';
			}
			else
			{
				$subdata[] ='<div class="CommentPopup"><a href="#removemegssage"  onclick="show_comments('.$comments['comments_id'].')" title="View Original Comment"><i class="fa fa-comment-o"></i></a>
			...
			elseif($comments['Status']=="R")
				$subdata[] = '<td><span data-toggle="tooltip" title="Rejected"><i class="fa fa-times"></i></span></td>';
			...
			/*$subdata[] ='<a class="button tick" href="#"   data-toggle="tooltip" title="Approve" comment_id = '.$comments['comments_id'].' status = '.$comments["Status"].'   id="status_change"><i id="status'.$comments['comments_id'].'" class="fa fa-check"></i></a>
			  ...
			  if(defined("USERACCESS_DELETE".$data['Menu_id']) && constant("USERACCESS_DELETE".$data['Menu_id']) == 1 && ($comments['Status']!="B") && ($comments['Status']!="R"))
			  {
...
class comment_status extends Comment_model
{
	public function statuschange($get_comment_id,$get_status,$user_comment,$modified,$userid)//to change status
	{
		...
		$status = $this->db->query("CALL Comment_updatestatus('".$get_status."','".$get_comment_id."','".addslashes($user_comment)."','".$modified."','".$userid."')");
 	// return true;
