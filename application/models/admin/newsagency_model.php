...
class getnewsagencydetails extends newsagency_model
{
	...
	public function newsagencydetails_delete($input_dpt_id)
	{
		...
		if($dpt_check_pro > 0) 
		{
			...
			redirect(base_url().'dmcpan/newsagency_manager');
		}
		else
		{
...
class datatable extends newsagency_model
{
	...
	public function newsagency_datatable()
	{
		...
		foreach($newsagency_manager as $newsagency) 
		{
			...
			$subdata[] ='<a class="button tick" href="'.base_url().'dmcpan/newsagency_manager/edit_details/'.$newsagency['Agency_id'].'" data-toggle="tooltip" title="Edit"> <i class="fa fa-pencil" ></i> </a>
