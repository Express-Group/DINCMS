...
class  Template_design_model extends CI_Model {
	...
	public function xml_backup($parameter){
		extract($parameter);
		$Previous_xml=$this->live_db->select('menuid,pagetype,workspace_version_id,published_templatexml,templateid,Header_Adscript,common_header,common_rightpanel,common_footer,Published_Version_Id,Modifiedby,Modifiedon')->from('page_master')->where(['menuid'=>$menu_id,'pagetype'=>$PageType])->get();
		$data=$Previous_xml->result();
		$SectionQuery=$this->db->select('Sectionname')->from('sectionmaster')->where('Section_id',$data[0]->menuid)->get();
		$SectionQuery=$SectionQuery->result();
		$SectionName=$SectionQuery[0]->Sectionname;
		$version_query=$this->db->query("SELECT Version_Name FROM page_template_versions WHERE Version_Id='".$data[0]->workspace_version_id."'")->result();
		$VersionName=$version_query[0]->Version_Name;
		if($data[0]->Modifiedby=='' || $data[0]->Modifiedby==null){ $data[0]->Modifiedby=0; }
		if($data[0]->templateid=='' || $data[0]->templateid==null){ $data[0]->templateid=0; }
		if($VersionName =='' || $VersionName==null){ $VersionName='0'; }
		if($data[0]->workspace_version_id=='' || $data[0]->workspace_version_id==null){ $data[0]->workspace_version_id=0; }
		$this->db->insert('version_master',['version_xml'=>$data[0]->published_templatexml,'header_advscript'=>$data[0]->Header_Adscript,'common_header'=>$data[0]->common_header,'common_rightpanel'=>$data[0]->common_rightpanel,'common_footer'=>$data[0]->common_footer,'published_version_id'=>$data[0]->Published_Version_Id,'version_id'=>$data[0]->workspace_version_id,'menu_id'=>$data[0]->menuid,'page_type'=>$data[0]->pagetype,'modified_by'=>$data[0]->Modifiedby,'section_name'=>$SectionQuery[0]->Sectionname,'Version_Name'=>$VersionName,'templateid'=>$data[0]->templateid]);
	}
...
class WidgetAddArticles extends Template_design_model
{
	...
	{		
		...
		$order_section_condition= ($section_id !='' && $is_have_section_mapping) ? " csp.content_id IN ( SELECT content_id FROM ".$section_mapping_table." WHERE Section_ID = ".$section_id." ORDER BY FIELD(Section_ID, ".$section_id.") DESC ),m.publish_start_date desc " : (($current_widget_section_id !='' && $is_have_section_mapping)? " csp.content_id IN ( SELECT content_id FROM ".$section_mapping_table." WHERE Section_ID = ".$current_widget_section_id." ORDER BY FIELD(Section_ID, ".$current_widget_section_id.") DESC ),m.publish_start_date DESC  " : " m.content_id desc ");
