...

{
...

	{
		...
		$null_value 		= "NULL";
			...
			if ($this->db->trans_status() === FALSE || $this->live_db->trans_status() === FALSE) {
				...
				//amp_validation($article_details['LiveArticleDetails']['url'] , $article_id ,'');
				//to save article page as html
				if($txtStatus == 'P'){
					/* $response = $this->file_get_contents_utf8("http://www.dinamani.com/".$article_details["url"]);
			...
			*/
			...
			$article_details['content_id'] = $article_id;
				...
				*/
				...
				//amp_validation($update_archive_details['url'] , $article_id ,$year);
	...
	*/
	public function update_live_article($article_details) {
		...
		$this->live_db->query('CALL update_article ('.$article_details["content_id"].','.$article_details["section_id"].',"'.addslashes($article_details["section_name"]).'",'.$article_details["parent_section_id"].',"'.addslashes($article_details["parent_section_name"]).'",'.$article_details["grant_section_id"].',"'.addslashes($article_details["grant_parent_section_name"]).'","'.$article_details["linked_to_columnist"].'","'.$article_details["publish_start_date"].'","'.$article_details["publish_end_date"].'","'.$article_details["last_updated_on"].'","'.addslashes($article_details["title"]).'","'.addslashes($article_details["url"]).'","'.addslashes($article_details["summary_html"]).'","'.addslashes($article_details['article_page_content_html']).'","'.addslashes($article_details["home_page_image_path"]).'","'.addslashes($article_details["home_page_image_title"]).'","'.addslashes($article_details["home_page_image_alt"]).'","'.addslashes($article_details["section_page_image_path"]).'","'.addslashes($article_details["section_page_image_title"]).'","'.addslashes($article_details["section_page_image_alt"]).'","'.addslashes($article_details["article_page_image_path"]).'","'.addslashes($article_details["article_page_image_title"]).'","'.addslashes($article_details["article_page_image_alt"]).'","'.addslashes($article_details["column_name"]).'","'.addslashes($article_details["tags"]).'",'.$article_details["allow_comments"].','.$article_details["allow_pagination"].',"'.addslashes($article_details["agency_name"]).'","'.addslashes($article_details["author_name"]).'","'.addslashes($article_details["author_image_path"]).'","'.addslashes($article_details["author_image_title"]).'","'.addslashes($article_details["author_image_alt"]).'","'.addslashes($article_details["country_name"]).'","'.addslashes($article_details["state_name"]).'","'.addslashes($article_details["city_name"]).'",'.$article_details["no_indexed"].','.$article_details["no_follow"].',"'.addslashes($article_details["canonical_url"]).'","'.addslashes($article_details["meta_Title"]).'","'.addslashes($article_details["meta_description"]).'",'.$article_details["section_promotion"].',"'.$article_details["status"].'","'.$article_details["amp_status"].'")');
	...
	{	
		...
		*/
		...
		if ($txtPublishEndDate != '')
			...
			if( $txtStatus == 'P' && ( $txtOldStatus == 'D' ||  $txtOldStatus == '' ) && folder_name == 'dmcpan') {
					$data['modifiedon'] = $data['PublishStartDate'];
			} else {
		...
		if (isset($cbAllowAmp) && $cbAllowAmp == 'on') $data['cbAllowAmp'] = 1;
		else $data['cbAllowAmp'] = 0;
		...
		$LiveArticleDetails['status'] 										= $txtStatus;
			...
			$LiveArticleDetails['amp_status'] 			=  $data['cbAllowAmp'];
	...
	*/
    ...
    default:
		...
		foreach($article_manager as $article) {
			...
			$mobilrUrl = explode('/',$article['url']);
			$mobStaticUrl = 'https://m.dinamani.com/article/';
			$uriPos = strrpos(end($mobilrUrl), "-");
			$uri = '/'.substr(end($mobilrUrl) , 0 , $uriPos).'/A'.date('Y' , strtotime($article['publish_start_date'])).'-'.$article['content_id'];
			//$subdata[] ='<p mobileUrl="'.$mobStaticUrl.@$mobilrUrl[0].$uri.'" class="tooltip_cursor pr" '.$Style.' title="'.strip_tags($article['title']).'"><span title="Copy Mobile Url" class="mob_url"><i class="fa fa-mobile" aria-hidden="true"></i></span>'.shortDescription(strip_tags($article['title'])).'</p>';
			...
			$subdata[] = $status_icon;
			/* if($article["status"]!="D" && $article["status"]!="U"):
				$subdata[] = '<span class="amp_'.$article['content_id'].'"  '.((@$article['amp_status']!=1) ? 'onclick="amp_validate('.$article['content_id'].')"':'').' style="background: #fff;border: 1px solid #d5cbcb;padding: 1px 7px 2px;   border-radius: 50px;color: '.((@$article['amp_status']==1) ? 'green':'red').';font-weight: bold;font-size: 15px;cursor:pointer;">AMP</span>';
			else:
				$subdata[] ='-';
			endif; */
			...
			if($CheckInYear <= $CurrentYear) {
				...
				if ($this->archive_db->table_exists($TableName)) {
					...
					$ArchiveRecordsFiltered = 0;
							...
							$this->archive_db->select("content_id,title,publish_start_date,status, url,articlepageimageid,homepageimageid,sectionpageimageid,author_name,modified_by,modified_on,amp_status");
					...
					if($ArchiveRecordsFiltered != 0 ){
							...
							$subdata[] = $status_icon;
							//$subdata[] = '<span class="amp_'.$article['content_id'].'"  '.((@$article['amp_status']!=1) ? 'onclick="amp_validate('.$article['content_id'].' , '.$CheckInYear.')"':'').'  style="background: #fff;border: 1px solid #d5cbcb;padding: 1px 7px 2px;   border-radius: 50px;color: '.((@$article['amp_status']==1) ? 'green':'red').';font-weight: bold;font-size: 15px;">AMP</span>';
							//$subdata[] = $article['Hits'];
							//$subdata[] = 0;
							...
							$set_status ='<div class="buttonHolder">';
								...
								if(defined("USERACCESS_EDIT".$Menu_id) && constant("USERACCESS_EDIT".$Menu_id) == 1){
									//$set_status .= '<a class="button tick tooltip-2"  href="'.base_url().folder_name.'/'.$edit_url.'" target="_blank" title="Edit"><i class="fa fa-pencil"></i></a><a class="button tick tooltip-2"  href="'.BASEURL.$article['url'].'?page=preview" target="_blank" title="Preview"><i class="fa fa-eye"></i></i></a>';
									$set_status .= '<a class="button tick tooltip-2"  href="'.base_url().folder_name.'/'.$edit_url.'" target="_blank" title="Edit"><i class="fa fa-pencil"></i></a><a class="button tick tooltip-2"  href="'.BASEURL.$article['url'].'" target="_blank" title="Preview"><i class="fa fa-eye"></i></i></a>';
								}
								else {
									$set_status .= '';
								}
