	...
	public function state_datatable()
	{
		...
		foreach($state_manager as $state) 
		{
			...
			$subdata[] = date("d-m-Y H:i:s",strtotime($state['Createdon']));
			  ...
			  $set_rights .= '<div><a class="button tick" href="'.base_url().'dmcpan/state_manager/edit_details/'.$state['State_Id'].'" data-toggle="tooltip" title="Edit"> <i class="fa fa-pencil" ></i> </a>';
			  } 
			  else 
			  { 
			  	$set_rights.="";
