	...
	public function get_weeklyPredictions()
	{
		...
		foreach($weeklyPrediction_values as $weekly_prediction) {
			...
			if(defined("USERACCESS_EDIT".$Menu_id) && constant("USERACCESS_EDIT".$Menu_id) == '1'){
				$set_rights .= '<div class="buttonHolder"><a class="button heart tooltip-3"  href="'.base_url().'dmcpan/astro_weekly_prediction/update_weekly_predictions/'.urlencode(base64_encode($weekly_prediction['prediction_id'])).'" data-toggle="tooltip" title="Edit"> <i class="fa fa-pencil" ></i> </a>';
			} else { $set_rights.="";			}
	...
	public function insert_update($user_id)
	{
		...
		if($hidden_id == "")
		{
			...
			if($this->db->trans_status() == FALSE)
			{
				...
				redirect('dmcpan/astro_weekly_prediction');
			}
			else
			{
