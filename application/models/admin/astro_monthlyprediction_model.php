	...
	public function get_monthlyPredictions()
	{
		...
		foreach($monthlyPrediction_values as $monthly_prediction) {
			...
			if(defined("USERACCESS_EDIT".$Menu_id) && constant("USERACCESS_EDIT".$Menu_id) == '1'){
				$set_rights .= '<div class="buttonHolder"><a class="button heart tooltip-3"  href="'.base_url().'dmcpan/astro_monthly_prediction/update_monthly_predictions/'.urlencode(base64_encode($monthly_prediction['prediction_id'])).$qry.'" data-toggle="tooltip" title="Edit"> <i class="fa fa-pencil" ></i> </a>';
			} else { $set_rights.="";			}
	...
	public function insert_update($user_id)
	{
		...
		if($hidden_id == "")
		{
			...
			if($this->db->trans_status() == FALSE)
			{
				...
				redirect('dmcpan/astro_monthly_prediction');
			}
			else
			{
