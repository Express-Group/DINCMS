<?php 
class numerology_weeklyPrediction_model extends CI_Model
{
	...
	{	
		$this->db->reconnect();
		$result =  $this->db->query("CALL get_numerology_weeklyPredictionById('".$prediction_id."')")->row_array();
		return $result;
	}
	...
	public function check_alreadyExists()
	{
		...
		$check_data = $this->db->query("CALL check_numerologyWeeklyPrediction_exists('".$number_id."', '".$post_date."', '".$prediction_id."')");
...
class weekly_predictions extends numerology_weeklyPrediction_model
{
	public function get_weeklyPredictions()
	{
		...
		$this->db->reconnect();
		$Total_rows = $this->db->query('CALL get_numerology_WeeklyPrediction("","","","")')->num_rows();
		...
		$weeklyPrediction_values =  $this->db->query('CALL get_numerology_WeeklyPrediction(" ORDER BY '.$order_field.' '.$order.' LIMIT '.$start.', '.$length.'","'.$from_date.'","'.$to_date.'","'.$number_id.'")')->result_array();

		$this->db->reconnect();

		$recordsFiltered =  $this->db->query('CALL get_numerology_WeeklyPrediction("","'.$from_date.'","'.$to_date.'","'.$number_id.'")')->num_rows();
		$data['draw'] = $draw;
		...
		$Menu_id = get_menu_details_by_menu_name('Weekly Prediction');
		...
		foreach($weeklyPrediction_values as $weekly_prediction) {
			...
			if(defined("USERACCESS_EDIT".$Menu_id) && constant("USERACCESS_EDIT".$Menu_id) == '1'){
				$set_rights .= '<div class="buttonHolder"><a class="button heart tooltip-3"  href="'.base_url().'dmcpan/numerology_weekly_prediction/update_weekly_predictions/'.urlencode(base64_encode($weekly_prediction['prediction_id'])).'" data-toggle="tooltip" title="Edit"> <i class="fa fa-pencil" ></i> </a>';
			} else { $set_rights.="";			}
...
class manipulate_weekly_predictions extends numerology_weeklyPrediction_model 
{
	public function delete_last($raasi_name,$dat)
    {  
	
	   $query = $this->db->query("CALL delete_last_record ('".$raasi_name."', '".$dat."','sel','numerology_weekly_predictions')")->result_array();
	   ...
	   $datt = end($created_on);
		 ...
		 $query1 = $this->db->query("CALL delete_last_record ('".$raasi_name."', '".$datt."','del','numerology_weekly_predictions')");
	...
	public function insert_update($user_id)
	{
		...
		if($hidden_id == "")
		{
			...
			$this->db->query("CALL numerology_weeklyPrediction_insert('".$number_id."', '".$Date."', '".$To_Date."', '".$txtPredictionDetails."','".$user_id."','".date('Y-m-d H:i:s')."','".$user_id."','".date('Y-m-d H:i:s')."')");
			$this->delete_last($raasi_name,$dat);
			...
			if($this->db->trans_status() == FALSE)
			{
				...
				redirect('dmcpan/numerology_weekly_prediction');
			}
			else
			{
		...
		else
		{
			...
			$this->db->query("CALL numerology_weeklyPrediction_update('".$number_id."', '".$Date."', '".$To_Date."', '".$txtPredictionDetails."','".$user_id."','".date('Y-m-d H:i:s')."','".$user_id."','".date('Y-m-d H:i:s')."', '".$hidden_id."')");
