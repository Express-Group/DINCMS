	...
	public function get_yearlyPredictions()
	{
		...
		foreach($yearlyPrediction_values as $yearly_prediction) {
			...
			if(defined("USERACCESS_EDIT".$Menu_id) && constant("USERACCESS_EDIT".$Menu_id) == '1'){
				$set_rights .= '<div class="buttonHolder"><a class="button heart tooltip-3"  href="'.base_url().'dmcpan/astro_yearly_prediction/update_yearly_predictions/'.urlencode(base64_encode($yearly_prediction['prediction_id'])).'" data-toggle="tooltip" title="Edit"> <i class="fa fa-pencil" ></i> </a>';
			} else { $set_rights.="";			}
	...
	public function insert_update($user_id)
	{
		...
		if($hidden_id == "")
		{
			...
			if($this->db->trans_status() == FALSE)
			{
				...
				redirect('dmcpan/astro_yearly_prediction');
			}
			else
			{
