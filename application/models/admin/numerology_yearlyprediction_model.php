<?php 
class numerology_yearlyPrediction_model extends CI_Model
{

	public function get_numerology_yearlypredictions()  
	{	
	...
	{	
		$this->db->reconnect();
		$result =  $this->db->query("CALL get_numerology_yearlyPredictionById('".$prediction_id."')")->row_array();
		return $result;
	}
	...
	public function check_alreadyExists()
	{
		...
		//error_log("CALL check_numerologyYearlyPrediction_exists('".$number_id."', '".$date_format."','".$prediction_id."')");
		$check_data = $this->db->query("CALL check_numerologyYearlyPrediction_exists('".$number_id."', '".$date_format."','".$prediction_id."')");
...
class yearly_predictions extends numerology_yearlyPrediction_model
{
	public function get_yearlyPredictions()
	{
		...
		$this->db->reconnect();
		$Total_rows = $this->db->query('CALL get_numerology_YearlyPrediction("","","","")')->num_rows();
		...
		$yearlyPrediction_values =  $this->db->query('CALL get_numerology_YearlyPrediction(" ORDER BY '.$order_field.' '.$order.' LIMIT '.$start.', '.$length.'","'.$from_date.'","'.$to_date.'","'.$number_id.'")')->result_array();

		$this->db->reconnect();

		$recordsFiltered =  $this->db->query('CALL get_numerology_YearlyPrediction("","'.$from_date.'","'.$to_date.'","'.$number_id.'")')->num_rows();
		$data['draw'] = $draw;
		...
		$Menu_id = get_menu_details_by_menu_name('Yearly Prediction');
		...
		foreach($yearlyPrediction_values as $yearly_prediction) {
			...
			if(defined("USERACCESS_EDIT".$Menu_id) && constant("USERACCESS_EDIT".$Menu_id) == '1'){
				$set_rights .= '<div class="buttonHolder"><a class="button heart tooltip-3"  href="'.base_url().'dmcpan/numerology_yearly_prediction/update_yearly_predictions/'.urlencode(base64_encode($yearly_prediction['prediction_id'])).'" data-toggle="tooltip" title="Edit"> <i class="fa fa-pencil" ></i> </a>';
			} else { $set_rights.="";			}
...
class manipulate_yearly_predictions extends numerology_yearlyPrediction_model 
{
	public function insert_update($user_id)
	{
		...
		if($hidden_id == "")
		{
			...
			$this->db->query("CALL numerology_yearlyPrediction_insert('".$number_id."', '".$Date."', '".$txtPredictionDetails."','".$user_id."','".date('Y-m-d H:i:s')."','".$user_id."','".date('Y-m-d H:i:s')."')");
			
			if($this->db->trans_status() == FALSE)
			{
				$this->db->trans_rollback();
				$this->session->set_flashdata('error', "Problem while inserting. Please try again");
				redirect('dmcpan/numerology_yearly_prediction');
			}
			else
			{
		...
		else
		{
			...
			$this->db->query("CALL numerology_yearlyPrediction_update('".$number_id."', '".$Date."','".$txtPredictionDetails."','".$user_id."','".date('Y-m-d H:i:s')."','".$user_id."','".date('Y-m-d H:i:s')."', '".$hidden_id."')");
