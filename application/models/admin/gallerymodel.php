	...
	*/
...

	{
		...
		extract($_POST);
			...
			$data['UrlTitle'] = mb_strtolower($data['UrlTitle']);
			$data['UrlTitle'] = mb_strtolower(join("",( explode("&nbsp;",$data['UrlTitle']))));
			if(!isset($ddAgency))
			$ddAgency = 0;
	...
	*/
    ...
    default:
		...
		foreach($gallery_manager as $gallery) {
			...
			$Firstname=$this->db->query("SELECT Username FROM usermaster WHERE User_id=(SELECT Modifiedby FROM gallerymaster WHERE content_id='".$gallery['content_id']."')")->result();
