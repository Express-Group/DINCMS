	...
	public function get_generalPredictions()
	{
		...
		foreach($generalPrediction_values as $general_prediction) {
			...
			if(defined("USERACCESS_EDIT".$Menu_id) && constant("USERACCESS_EDIT".$Menu_id) == '1'){
				$set_rights .= '<div class="buttonHolder"><a class="button heart tooltip-3"  href="'.base_url().'dmcpan/astro_general_prediction/update_general_predictions/'.urlencode(base64_encode($general_prediction['section_id'])).'" data-toggle="tooltip" title="Edit"> <i class="fa fa-pencil" ></i> </a>';
			} else { $set_rights.="";			}
	...
	public function insert_update($user_id)
	{
		...
		if($hidden_id == "")
		{
			...
			if($this->db->trans_status() == FALSE)
			{
				...
				redirect('dmcpan/astro_general_prediction');
			}
			else
			{
