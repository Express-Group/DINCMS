<?php 
class numerology_monthlyPrediction_model extends CI_Model
{
	...
	{	
		$this->db->reconnect();
		$result =  $this->db->query("CALL get_numerology_monthlyPredictionById('".$prediction_id."')")->row_array();
		return $result;
	}
	...
	public function check_alreadyExists()
	{
		...
		$check_data = $this->db->query("CALL check_numerologyMonthlyPrediction_exists('".$number_id."','".$month."' , '".$date_format ."', '".$prediction_id."')");
		...
		$check_data = $this->db->query("CALL check_numerologytamilMonthlyPrediction_exists('".$number_id."','".$predict_start_date."', '', '".$prediction_id."')");
...
class monthly_predictions extends numerology_monthlyPrediction_model
{
	public function get_monthlyPredictions()
	{
		...
		$call_procedure  = ($language=='english') ? 'get_numerology_MonthlyPrediction': 'get_numerology_MonthlyPrediction_tamil';
		$this->db->reconnect();
		...
		$monthlyPrediction_values =  $this->db->query('CALL '.$call_procedure.'(" ORDER BY '.$order_field.' '.$order.' LIMIT '.$start.', '.$length.'","'.$from_date.'","'.$to_date.'","'.$number_id.'")')->result_array();

		$this->db->reconnect();

		$recordsFiltered =  $this->db->query('CALL '.$call_procedure.'("","'.$from_date.'","'.$to_date.'","'.$number_id.'")')->num_rows();
		$data['draw'] = $draw;
		$data["recordsTotal"] = $Total_rows;
  		$data["recordsFiltered"] = $recordsFiltered ;
		$data['data'] = array();
		$Count = 0;
		$menu_name  = ($language=='english') ? 'Monthly Prediction': 'Numerology Monthly Tamil';
		...
		foreach($monthlyPrediction_values as $monthly_prediction) {
			...
			if(defined("USERACCESS_EDIT".$Menu_id) && constant("USERACCESS_EDIT".$Menu_id) == '1'){
				$set_rights .= '<div class="buttonHolder"><a class="button heart tooltip-3"  href="'.base_url().folder_name.'/numerology_monthly_prediction/update_monthly_predictions/'.urlencode(base64_encode($monthly_prediction['prediction_id'])).$qry.'" data-toggle="tooltip" title="Edit"> <i class="fa fa-pencil" ></i> </a>';
			} else { $set_rights.="";			}
...
class manipulate_monthly_predictions extends numerology_monthlyPrediction_model 
{
	...
	$dat =date('Y-m-d',$dat); 
	//echo $dat ;exit;
	    $query = $this->db->query("CALL delete_last_record ('".$raasi_name."', '".$dat."','sel','numerology_monthly_predictions')")->result_array();
	...
	//print_r($query);exit;
	  ...
	  $datt = end($created_on);
		 ...
		 $query1 = $this->db->query("CALL delete_last_record ('".$raasi_name."', '".$datt."','del','numerology_monthly_predictions')");
	...
	public function insert_update($user_id)
	{
		...
		if($hidden_id == "")
		{
			...
			$this->db->query("CALL numerology_monthlyPrediction_insert('".$number_id."', '".$prediction_month."', '".$prediction_date."', '".$txtPredictionDetails."','".$user_id."','".date('Y-m-d H:i:s')."','".$user_id."','".date('Y-m-d H:i:s')."')");
			//$this->delete_last($raasi_name,$dat);
			if($this->db->trans_status() == FALSE)
			{
				$this->db->trans_rollback();
				$this->session->set_flashdata('error', "Problem while inserting. Please try again");
				redirect('dmcpan/numerology_monthly_prediction');
			}
			else
			{
		...
		else
		{
			...
			$this->db->query("CALL numerology_monthlyPrediction_update('".$number_id."', '".$prediction_month."', '".$prediction_date."', '".$txtPredictionDetails."','".$user_id."','".date('Y-m-d H:i:s')."','".$user_id."','".date('Y-m-d H:i:s')."', '".$hidden_id."')");
	...
	public function insert_update_tamil_prediction($user_id)
	{
		...
		if($hidden_id == "")
		{
			...
			$this->db->query("CALL numerology_tamilmonthlyPrediction_insert('".$number_id."', '".$Date."', '".$To_Date."', '".$txtPredictionDetails."','".$user_id."','".date('Y-m-d H:i:s')."','".$user_id."','".date('Y-m-d H:i:s')."', 2)");
			...
			if($this->db->trans_status() == FALSE)
			{
				$this->db->trans_rollback();
				$this->session->set_flashdata('error', "Problem while inserting. Please try again");
				redirect(folder_name.'/numerology_monthly_prediction/tamil');
			}
			else
			{
				$this->db->trans_commit();
		...
		else
		{
			...
			$this->db->query("CALL numerology_tamilmonthlyPrediction_update('".$number_id."', '".$Date."', '".$To_Date."', '".$txtPredictionDetails."','".$user_id."','".date('Y-m-d H:i:s')."','".$user_id."','".date('Y-m-d H:i:s')."', '".$hidden_id."')");
