...
class detailsdpt extends topic_model
{
	public function addtopicdetails($user_id)//fn to list datatable details
	{
			...
			//$txtDescription= htmlspecialchars(trim($this->input->post('txtDescription')));
			//$txtDescription = addslashes(str_replace("'", "&#039;", $txtDescription));
			...
			{
				$this->db->trans_begin();
				$column_insert_pro = $this->db->query("CALL column_insert('".$topicname."','".$user_id."','".$user_id."','".$createdon."' ,'".$txtDescription."')"); 
			...
			else
			{
				...
				$column_update_pro = $this->db->query("CALL column_update('".$topicname."','".$user_id."','".$modifiedon."','".$hdn_topic_id."','".$txtDescription."')"); 
...
class gettopicdetails extends topic_model
{
	...
	public function topicdetails_delete($input_dpt_id)
	{
		...
		if($dpt_check_pro > 0) 
		{
			...
			redirect(base_url().'dmcpan/topic_manager');
		}
		else
		{
	...
	public function topic_datatable()
	{
		...
		foreach($topic_manager as $topic) 
		{
			...
			/*$subdata[] ='<a class="button tick" href="'.base_url().'admin/topic_manager/edit_details/'.$topic['topic_id'].'" data-toggle="tooltip" title="Edit"> <i class="fa fa-pencil" ></i> </a>
			  ...
			  $set_rights .= '<div><a class="button tick" href="'.base_url().'dmcpan/topic_manager/edit_details/'.$topic['column_id'].'" data-toggle="tooltip" title="Edit"> <i class="fa fa-pencil" ></i> </a>';
			  } 
			  else 
			  { 
			  	$set_rights.="";
