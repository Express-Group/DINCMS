   ...
   public function fill_widget($widget_location ,$content)
	{			
     ...
     $widget_instance_details    = $this->widget_model->getWidgetInstance('', '','', '', $pass_widget_instance, $content['mode']);	 
		...
		$widget_instance_details =	array('isSummaryRequired'=> (string)$content['widget_values']['cdata-showSummary'],'status'=> (string)$content['widget_values']['cdata-widgetStatus'],'publish_start_date'=> (string)$content['widget_values']['cdata-widgetpublishOn'],'publish_end_date'=> (string)$content['widget_values']['cdata-widgetpublishOff'],'WidgetSection_ID'=> ((string)$content['widget_values']['cdata-widgetCategory']=='undefined')? '': (string)$content['widget_values']['cdata-widgetCategory'],'CustomTitle'=> ((string)$content['widget_values']['cdata-customTitle']=='undefined')? '' : (string)$content['widget_values']['cdata-customTitle'],'Maximum_Articles'=> (string)$content['widget_values']['cdata-customMaxArticles'],'Background_Color'=>' ','RenderingMode'=> (string)$content['widget_values']['cdata-renderingMode']);
		}
	 ...
	 $b_section_inc = 0;
		$templateString['view_templ'] = '';
				...
				foreach($values->widgetcontainer as $ckey => $cvalues )
				{
					...
					foreach($cvalues->widget as $wkey => $wvalues)
					{
					//	echo "<br>wvalues---contao9in".$wvalues;
					//	print_r($wvalues);
						$c_span_val = $container_layout[$c_inc];	
						$padding_zero = "";
						$xs_val = "12";
						...
						{
							$templateString['view_templ'] .= '<div class="widget-container widget-container-' .  $wvalues['id'] . '">';		
							$allow_comments  = ($content_from=="preview")? @$content_details[0]['Allowcomments'] : @$content_details[0]['allow_comments'];
							$content_details = ($wvalues['data-widgetfilepath']!="admin/widgets/article_details")? array('comments'=>$allow_comments)  : $content_details;			
