...
IExp.templateManager = function () {
	var TemplateManagerObj = { 			
		...
		publish_template: function(){
				//added by krishwer to stop publishing articles and templates
				var current_time=new Date();
				var Hours=current_time.getHours();
				var Minutes=current_time.getMinutes();
				var Seconds=current_time.getSeconds();
				var TempMinutes=15;
				if((Hours==3 || Hours==6 || Hours==9 || Hours==12 || Hours==15 || Hours==18 || Hours==21)&& Minutes >=1 &&  Minutes <= 15){
					var Milliseconds= (TempMinutes - Minutes)*60*1000;
					console.log(Milliseconds);
					toastr.options = {"progressBar": true,"positionClass": "toast-top-center","closeButton": true,"timeOut": Milliseconds};
					toastr["warning"](Hours+":00 to "+Hours+":15 Scheduled for backup. try after "+Hours+":15"+"<div style='width:100%;text-align:center;'>wait time : 00:"+(TempMinutes - Minutes)+" mins</div>");
					return false;
				}
				/*---end of code-----*/
					...
					$.fancybox({						
						...
						'afterShow': function () {								
							...
							if (widgetObject.data("customConfig")) {
								...
								($("#widgetTitle").length && $("#widgetTitle").length > 0 && !(jQuery.type( additionalProperties.customTitle ) === "string" && additionalProperties.customTitle === "undefined") )  ? $("#widgetTitle").val($.trim(additionalProperties.customTitle)) : '';
								...
								(additionalProperties.widgetStatus !='' && additionalProperties.widgetStatus !='undefined') ?$("input[name='status'][value='" + additionalProperties.widgetStatus + "']").attr('checked','checked') : $("input[name='status'][value='1']").attr('checked','checked');
