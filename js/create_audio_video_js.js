...
$("#txtMetaTitle").blur(function() {
 ...
 $("#SubSection"+$(this).attr('val')).toggle();
			...
			if($("input:checkbox[name='version[]']:checked").length == 2)
			{
				...
				count = 0;
				 ...
				 if(version_array[0] != '' && version_array[1] != '')
				 {
						...
						$.ajax({
								url: base_url+"dmcpan/audio_video_manager/view_version_compare", // Url to which the request is send
								type: "POST",             // Type of request to be send, called as method
								data:  postdata,
								dataType: "HTML",
								success: function(data)   // A function to be called if request succeeds
 	...
 	$("#publish_history").click(function()
	{
			...
			$.ajax({
					url: base_url+"dmcpan/audio_video_manager/get_publish_history_popup", // Url to which the request is send
					type: "POST",             // Type of request to be send, called as method
					data:  postdata,
					dataType: "HTML",
					async: false, 
	...
	$("#txtbyline").keyup(function()
	{
		if($("#agency_id").val() != '') 
		{
			...
			var availableTags = base_url+"dmcpan/common/get_author_name/"+agency_id;
	...
	$("#txtState").keyup(function()
	{
		if($("#country_id").val() != '') 
		{
					...
					var availableTags = base_url+"dmcpan/common/get_state_name/"+country_id;
	...
	$("#txtCity").keyup(function()
	{
		if($("#country_id").val() != '' && $("#state_id").val() != '') 
		{
					...
					var availableTags = base_url+"dmcpan/common/get_city_name/"+country_id+"/"+state_id;
	...
	if($("#country_id").val() != '')
		  {
			   ...
			   $.ajax({
			   url: base_url+"dmcpan/common/get_state", // Url to which the request is send
			   type: "POST",             // Type of request to be send, called as method
			   data:  postdata,
			   dataType: "json",
			   success: function(data)   // A function to be called if request succeeds
	 ...
	 if($("#country_id").val() != '' && $("#state_id").val() != '' )
	  {
			...
			$.ajax({
			url: base_url+"dmcpan/common/get_city", // Url to which the request is send
			type: "POST",             // Type of request to be send, called as method
			data:  postdata,
			dataType: "json",
			success: function(data)   // A function to be called if request succeeds
...
function version_publish(content_id,contentversion_id) 
{
	if(content_id != '' && contentversion_id != '')
	{
		...
		$.ajax({
				url: base_url+"dmcpan/common/version_publish", // Url to which the request is send
				type: "POST",             // Type of request to be send, called as method
				data:  postdata,
				dataType: "json",
				success: function(data)   // A function to be called if request succeeds
...
{  
	  ...
	  {
		  $.ajax({
			  url: base_url+"dmcpan/audio_video_manager/view_version", // Url to which the request is send
			  type: "POST",             // Type of request to be send, called as method
