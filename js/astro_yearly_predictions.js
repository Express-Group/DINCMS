	...
	$("#btnYearlyPrediction").click(function() 
	{
		if($("#yearlyPredictions_form").valid())
		{
			...
			$.ajax({
				...
				url: base_url+"dmcpan/astro_yearly_prediction/check_alreadyExists",
	...
	$(document.body).on('click',"#browse_image_insert",function() {
		...
		if(popuptype == 'bodytext') {
			...
			{		
				...
				$.ajax({
					url		: base_url+"dmcpan/common/get_image_by_content_ajax",
					type	: "POST",
					data	: ImageData,
					dataType: "HTML",
					async	: false, 	
		...
		{		
			if($("#browse_image_id").val() != '' && $("#browse_image_id").val() != 0 ) 
			{
				if($("#browse_image_id").data('image_source')) 
				{
					...
					$.ajax({
						url		: base_url+"dmcpan/astro_image/Insert_temp_from_image_library",
						type	: "POST",
						data	: ImageData,
						dataType: "json",
						async	: false, 	
...
function uploadFormData(formData) {
	...
	$.ajax({
		url: base_url+"dmcpan/astro_image/image_upload",
		type: "POST",
		data: formData,
		contentType:false,
		cache: false,
