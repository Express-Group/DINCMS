					...
					if(count)
					{
						...
						else // Alternative yandex
						{
							...
							$.getScript( 'http://share.yandex.ru/gpp.xml?url=' + $URL + '&callback=?', function(){
								...
								var gplus_count = parseInt(gshare);
				...
				default: // Default Facebook share
					...
					if(count)
					{
						...
						$.getJSON('http://graph.facebook.com/?id=' + $URL, function( fbdata ) {
							if (fbdata.shares > 999 && fbdata.shares <= 999999) {
								fb_count = parseFloat(fbdata.shares / 1000).toFixed(1)+'K';
							} else if (fbdata.shares > 999999) {
							   fb_count = parseFloat(fbdata.shares / 1000000).toFixed(1)+'M';
							} else {
								fb_count = fbdata.shares;
							}
