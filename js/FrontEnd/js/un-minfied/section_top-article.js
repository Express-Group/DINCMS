...
function mail_form_validate() {
	...
	var error_free=false;
		...
		$.ajax({
			...
			var mail_share_count= $(".PrintSocial1 .csbuttons-count:eq(0)").text();
				   if(mail_share_count == ''){
					   var mail_share_count= 0;
				   }else{
				    var mail_share_count = mail_share_count.replace(/[\(\)-]/g, "");
				   }
			$(".PrintSocial1 .csbuttons-count:eq(0)").text((parseInt(mail_share_count)+1));
			}
			});
		return false;
	}
...
function update_hits()
{
	var formData = {
		...
		$.ajax({
			...
			dataType    : 'json',
			success		: function(result){
			$(".PrintSocial1 .csbuttons-count:eq(0)").text(parseInt(result.emailed));
			},
		});
}