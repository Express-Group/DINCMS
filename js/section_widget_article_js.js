	...
	/*	if(main_section_text == 'Nation') {
		...
		$("#count1").blur(function() {
			...
			$("#suggestion_div").empty();
				$("#suggestion_div").show();
					$.ajax({
						url: base_url+"dmcpan/common/get_tags_by_meta_title", // Url to which the request is send
		...
		//alert($('#customImage_' + WidgetCustomImage.content_id + '_view').attr('my_attr') );
				...
				$.ajax({
					url: base_url+"dmcpan/section_widget_articles/custom_image_upload",
					type: "POST",					
					data: formData,
					contentType:false,
					cache: false,
					...
					success: function(data){
						...
						$("#orig_name" + WidgetCustomImage.content_id).val(data.orig_name);
						$("#full_path" + WidgetCustomImage.content_id).val(data.full_path);
						$("#filename" + WidgetCustomImage.content_id).val(data.physical_name);
						$("#temp_name" + WidgetCustomImage.content_id).val(data.file_name);
						$('#image_library_id'+ WidgetCustomImage.content_id).val('');
						$('#uploaded_image_'+ WidgetCustomImage.content_id).val('');
						
						$('#custom_image_old_id'+ WidgetCustomImage.content_id).val('');
						...
						$("#home_image_caption"+ WidgetCustomImage.content_id).attr("readonly", false);
						$("#home_image_alt"+ WidgetCustomImage.content_id).attr("readonly", false);
		...
		$(document.body).on('click',"#browse_image_insert",function() {			
			...
			var contentType = '1';
				...
				ImageIndex = $("#Image_content_id" + WidgetCustomImage.content_id).val();
				
				if(ImageIndex != '') {
					
					$("#home_physical_name"+ WidgetCustomImage.content_id).attr('readonly',false);
					
					window.open(base_url+"dmcpan/section_widget_articles/custom_image_processing/"+encodeURIComponent(base64_encode(ImageIndex))+"/"+WidgetCustomImage.support_image_width+"/"+WidgetCustomImage.support_image_height, '_blank');		
			...
			$(document.body).on('click', '#delete_article_image' ,function(){
				...
				if(ImageIndex != '') {
					...
					$.ajax({
						url: base_url+"dmcpan/article_image/delete_temp_image",
						type: "POST",
						data: ImageData,
						dataType: "json",
						async: false, 
			...
			$('#example').dataTable( {
				...
				"ajax": {
					"url": base_url+"dmcpan/article/search_internal_article",
					"type" : "POST",
					"data" : {
					...
					if(reached_last){					
						...
						if($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight){
							$container.append('<div class="LoadingSelectImageLocal"><img src="'+ base_url +'images/admin/loadingimage.gif"><br>Loading ...</div>');							
								$.ajax({
									url: base_url+"dmcpan/section_widget_articles/search_image_library_scroll/"+ scroll_down_inc, // Url to which the request is send
									type: "POST",             // Type of request to be send, called as method
									dataType: "HTML",
									complete: 	function(){													
			...
			success	: function(data) {
							if(is_from_save)
							{
								...
								$("#home_image_caption" + WidgetCustomImage.content_id).val(data.caption);											
									...
									$("#home_image_caption"+ WidgetCustomImage.content_id).attr("readonly", true);
									$("#home_image_alt"+ WidgetCustomImage.content_id).attr("readonly", true);											

								$("#home_image_alt" + WidgetCustomImage.content_id).val(data.alt);
								$('#Image_content_id'+ WidgetCustomImage.content_id).val(data.image_id);
									
								$('#image_library_id'+ WidgetCustomImage.content_id).val(data.imagecontent_id);
								
								$('#orig_name'+WidgetCustomImage.content_id).val('');	
								$('#full_path'+WidgetCustomImage.content_id).val('');	
								$('#filename'+WidgetCustomImage.content_id).val('');	
								$('#temp_name'+WidgetCustomImage.content_id).val('');
							...
							else
							{
								...
								$('#custom_image_old_id'+ WidgetCustomImage.content_id).val('');
								$('#orig_name'+WidgetCustomImage.content_id).val(data.orig_name);		
								$('#full_path'+WidgetCustomImage.content_id).val(data.source);		
								$('#filename'+WidgetCustomImage.content_id).val(data.physical_name);		
								$('#temp_name'+WidgetCustomImage.content_id).val(data.temp_name);
							}
							remove_image = false;
